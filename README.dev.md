...
The `v0.3.0` release finalized the first ToolX CLI product chain. `v0.3.2` is
the compatible reliability patch for deterministic CLI proxy control, FSXJ3
write-ahead journal recovery, and dedicated OpenSSL loopback TLS coverage.
`v0.2.0`
...
`--remote-url`, `--no-proxy-from-env`, and `--dry-run`, and reports
`schema_issues` and `proxy_from_environment` additively. The
`toolx_sync_cli_scenario` and `toolx_sync_proxy_contracts` tests cover publish,
overlay composition, snapshot/log/journal outputs, dry-run reports, validation,
and remote proxy routing.
...
network access. It covers default environment-proxy behavior and explicit
`--no-proxy-from-env` bypasses.
...
Before tagging `v0.3.2`:

- CI is green on Linux GCC, Linux Clang, Windows MSVC, macOS Clang, and the
  Linux/OpenSSL `httpx_tests` job.
...
- `docs/releases/v0.3.2.md` and [docs/productization-retro.md](docs/productization-retro.md)
...
Keep both backends off for the default release package. CI separately configures
Linux/OpenSSL and runs `httpx_tests`; its loopback test generates the private
key at runtime and writes only a temporary public CA certificate.
