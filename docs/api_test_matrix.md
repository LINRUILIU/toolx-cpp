...
| `fsx` | batch plan/run/recover、walk/diff/sync、archive/link/watcher | R2 | T0/T1/T2 | T0 surface；T1 batch/walk/archive/link；T2 invalid sync roots、journal open failure, tar traversal rejection | `tests/api_surface_tests.cpp`, `tests/fsx_tests.cpp` | Covered |
...
| `httpx` | client send, retry, redirect, proxy, download/upload, TLS guard | R2/R3 | T0/T1/T2/T3 | T0 surface；T1 local HTTP/redirect/cookie/proxy; T2 retry conflict, env proxy/no_proxy, download/upload failure boundaries; T3 TLS backend guard | `tests/api_surface_tests.cpp`, `tests/httpx_tests.cpp` | Guarded |
...
| 模块 | 项目 | 状态 | 原因 |
| --- | --- | --- | --- |
| `httpx` | 真实外部 HTTPS/TLS 互操作 | Guarded | 默认测试不得依赖外网或本机 CA/backend；只测 backend option validation/capability behavior |
