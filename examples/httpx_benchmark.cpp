...
#else
        const int n = static_cast<int>(::send(client, payload.data() + sent, payload.size() - sent, 0));
#endif
        if (n <= 0)
        {
            return false;
...
int main(int argc, char** argv)
{
    ...
    std::cout << "httpx_benchmark start"
              << " total_requests=" << total_requests << " concurrency=" << concurrency << " url=" << url << "\n";
    ...
    std::cout << "httpx_benchmark done"
              ...
              << " handled_by_server=" << server.Handled() << " elapsed_ms=" << elapsed_ms
              << " qps=" << static_cast<std::uint64_t>(qps) << " p50_ms=" << Percentile(all_latencies, 0.50)
              << " p95_ms=" << Percentile(all_latencies, 0.95) << " p99_ms=" << Percentile(all_latencies, 0.99) << "\n";
