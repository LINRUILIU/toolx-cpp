...
struct ParseResult
{
    ...
    int exit_code{2}; // 解析结果对应的退出代码，默认值为 2，表示解析错误。调用方可以根据需要设置为其他值，如 0
                      // 表示成功，1 表示一般错误等。
    std::optional<ParseError> error; // 解析失败时的错误信息，包含错误类型、相关字段、原始输入和详细消息。
    // 解析得到的选项和值的映射，key 是选项或位置参数的名称，value 是一个字符串列表，
    // 包含所有出现的值（便于实现 RepeatMode 和 Count 语义）。
    ...
    std::string GetString(std::string_view key, std::string_view fallback = "")
        const; // 获取某个选项或位置参数的值并转换为字符串，如果不存在则返回 fallback。
    int GetInt(std::string_view key,
               int fallback = 0) const; // 获取某个选项或位置参数的值并转换为整数，如果不存在或转换失败则返回 fallback。
    double GetDouble(std::string_view key, double fallback = 0.0)
        const; // 获取某个选项或位置参数的值并转换为双精度浮点数，如果不存在或转换失败则返回 fallback。
    bool GetBool(std::string_view key, bool fallback = false)
        const; // 获取某个选项或位置参数的值并转换为布尔值，如果不存在或转换失败则返回 fallback。
    int GetCount(std::string_view key)
        const; // 获取某个选项或位置参数的出现次数，适用于 RepeatMode::Count 或 BoolFlagMode::Count 的选项。
};
...
class SubcommandRouter
{
  public:
    using Handler =
        std::function<int(const std::vector<std::string>&)>; // 子命令处理函数类型，接受子命令参数列表，返回退出代码。

    void Register(std::string name, Handler handler, std::string description = ""); // 注册一个子命令及其处理函数。
    int Dispatch(const std::vector<std::string>& args, std::string* error = nullptr)
        const; // 根据子命令名称分发到对应的处理函数，args[0] 应为子命令名称，后续为子命令参数。
...
{
  public:
    // 未知选项处理函数类型，接受未知选项的原始文本和一个输出错误消息的指针，
    // 返回是否成功处理（true 表示已处理，false 表示未处理且应视为错误）。
    using UnknownOptionHandler = std::function<bool(std::string_view token, std::string* error_message)>;
    ...
    OptionBuilder Option(std::string long_name,
                         char short_name = '\0'); // 定义一个选项，返回 OptionBuilder 以便链式设置选项属性。
    OptionBuilder
    Flag(std::string long_name,
         char short_name =
             '\0'); // 定义一个布尔 flag 选项，等同于 Option(...).BoolFlag()，返回 OptionBuilder 以便链式设置选项属性。
    ...
    ParseResult
    Parse(int argc,
          const char* const argv[]) const; // 解析命令行参数，返回 ParseResult 包含解析结果、错误信息和解析得到的值。
  ...
  public:
    class OptionBuilder
    {
      public:
        ...
        OptionBuilder&
        ConvertWith(ValueConverter converter);               // 为该选项设置局部 converter（优先级高于全局 converter）。
        ...
        OptionBuilder& Range(double min_value, double max_value,
                             RangePolicy policy = RangePolicy::Fail); // 设置数值选项的范围和超出范围时的处理策略。
    ...
    class PositionalBuilder
    {
      public:
        ...
        PositionalBuilder&
        ConvertWith(ValueConverter converter); // 为该位置参数设置局部 converter（优先级高于全局 converter）。
        ...
        PositionalBuilder& Range(double min_value, double max_value,
                                 RangePolicy policy = RangePolicy::Fail); // 设置位置参数的范围和超出范围时的处理策略。
  ...
  private:
    void ValidateOptionSpecOrThrow(const OptionSpec& spec, std::size_t index)
        const; // 验证选项规范是否合法，若不合法则抛出 std::invalid_argument 异常。
    void ValidatePositionalSpecOrThrow(const PositionalSpec& spec, std::size_t index)
        const; // 验证位置参数规范是否合法，若不合法则抛出 std::invalid_argument 异常。
    void ValidateConfigurationOrThrow()
        const; // 验证整个解析器配置是否合法（如互斥组和依赖规则），若不合法则抛出 std::invalid_argument 异常。
    ParseResult ApplyConstraintPipeline(ParseResult result) const; // 执行约束规则管线（内建规则 + 自定义规则）。
    ParseResult Fail(ParseResult result, ParseErrorKind kind, std::string field, std::string token,
                     std::string message) const; // 构造一个失败的 ParseResult，包含错误信息。
    void Trace(ParseResult& result, std::string stage, std::string token,
               std::string detail) const; // 追加 trace 事件。

    std::string program_name_; // 程序名称，用于帮助文本和错误信息中显示。
    mutable std::string
        runtime_program_name_;  // 运行时程序名称，默认为 program_name_，但在子命令调度时会更新为实际调用的子命令名称。
