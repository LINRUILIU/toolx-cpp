...
class ThreadPool
{
  public:
    ...
    template <typename Rep, typename Period> Status WaitForIdleFor(std::chrono::duration<Rep, Period> timeout)
    {
    ...
    Result<std::uint64_t> ScheduleEvery(std::chrono::steady_clock::duration interval, std::function<void()> task,
    ...
    {
        return SubmitWithPriority(TaskPriority::Normal, std::forward<Fn>(fn), std::forward<Args>(args)...);
    }
        ...
        -> Result<std::future<std::invoke_result_t<Fn, Args...>>>
    {
        ...
        Status status =
            EnqueueTask([task]() mutable { (*task)(); }, std::nullopt, ShouldWaitOnBackpressure(), priority);
        if (!status.ok)
        {
            out.ok = false;
    ...
    {
        return TrySubmitWithPriority(TaskPriority::Normal, std::forward<Fn>(fn), std::forward<Args>(args)...);
    }
        ...
        -> Result<std::future<std::invoke_result_t<Fn, Args...>>>
    {
        ...
        Result<std::future<ReturnType>> out;
        std::future<ReturnType> future = task->get_future();
        Status status = EnqueueTask([task]() mutable { (*task)(); }, std::nullopt, false, priority);
        if (!status.ok)
        {
            out.ok = false;
            out.error = std::move(status.error);
    ...
    auto SubmitUntil(std::chrono::steady_clock::time_point deadline, Fn&& fn, Args&&... args)
        -> Result<std::future<std::invoke_result_t<Fn, Args...>>>
    {
        return SubmitWithPriorityUntil(deadline, TaskPriority::Normal, std::forward<Fn>(fn),
                                       std::forward<Args>(args)...);
    }

    template <typename Fn, typename... Args>
    auto SubmitWithPriorityUntil(std::chrono::steady_clock::time_point deadline, TaskPriority priority, Fn&& fn,
                                 Args&&... args) -> Result<std::future<std::invoke_result_t<Fn, Args...>>>
    {
        using ReturnType = std::invoke_result_t<Fn, Args...>;
        ...
        Result<std::future<ReturnType>> out;
        std::future<ReturnType> future = task->get_future();
        Status status = EnqueueTask([task]() mutable { (*task)(); }, deadline, true, priority);
        if (!status.ok)
        {
            out.ok = false;
            out.error = std::move(status.error);
...
template <typename T>
Result<std::size_t> WaitAnyUntil(std::vector<std::future<T>>& futures, std::chrono::steady_clock::time_point deadline)
{
...
template <typename T, typename Rep, typename Period>
Result<std::size_t> WaitAnyFor(std::vector<std::future<T>>& futures, std::chrono::duration<Rep, Period> timeout)
{
    const auto casted = std::chrono::duration_cast<std::chrono::steady_clock::duration>(timeout);
    const auto deadline = sysx::time::SteadyNow() + casted;
...
template <typename T> Result<std::size_t> WaitAny(std::vector<std::future<T>>& futures)
{
    Result<std::size_t> out;
    if (futures.empty())
    {
