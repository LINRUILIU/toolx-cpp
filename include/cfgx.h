...
using RemoteFetcher =
    std::function<Result<RemoteFetchResponse>(const RemoteFetchRequest& request)>; // 可插拔远程拉取回调。
...
class Node
{
  public:
    ...
    Status SetAt(std::size_t index, Node value,
                 bool auto_expand =
                     true); // 设置数组元素，如果auto_expand为true且索引越界则自动扩展数组，如果类型不匹配则返回错误状态
  ...
  private:
    using Storage = std::variant<std::monostate, bool, std::int64_t, double, std::string, Object,
                                 Array>; // 存储节点值的变体类型，支持多种类型的值
    // std::monostate表示空节点
...
struct ParserAdapter
{
    ...
    std::function<Result<std::string>(const Node& root, ConfigFormat format, int indent)>
        dump; // 将节点转储为指定格式的文本
};
...
Result<std::vector<PathToken>>
ParsePath(std::string_view path); // 解析路径字符串为路径令牌列表，支持键分隔符、数组索引和转义字符

Result<const Node*> GetNode(const Node& root,
                            std::string_view path); // 根据路径获取节点，如果路径无效或类型不匹配则返回错误
Result<Node*> GetNodeMutable(Node& root,
                             std::string_view path); // 根据路径获取可变节点，如果路径无效或类型不匹配则返回错误
...
Status Merge(Node& base, const Node& overlay,
             bool append_arrays =
                 false); // 合并两个节点，根据类型进行不同的合并策略，支持对象递归合并、数组覆盖或追加，以及混合类型覆盖

Result<Node>
BuildEnvLayerFromPairs(const std::vector<std::pair<std::string, std::string>>& variables,
                       std::string_view prefix = "APP_CFG_",
                       const TypePolicyOptions& policy =
                           {}); // 从环境变量对构建配置层，使用指定的前缀过滤环境变量，并根据类型策略选项解析值
Result<Node> BuildEnvLayerFromEnvironment(
    std::string_view prefix = "APP_CFG_",
    const TypePolicyOptions& policy = {}); // 从当前环境构建配置层，使用指定的前缀过滤环境变量，并根据类型策略选项解析值

Result<Node> ComposeLayers(
    const Node& base, const std::optional<Node>& env_layer = std::nullopt,
    const std::optional<Node>& local_layer = std::nullopt, const RuntimeOverrides* runtime = nullptr,
    const ComposeOptions& options = {}, std::vector<SourceAttribution>* source_trace = nullptr,
    const std::optional<Node>& remote_layer = std::
        nullopt); // 合成多个配置层，按照优先级顺序进行合成，并生成来源追踪列表，支持基本层、环境层、远程层、本地层和运行时覆盖，以及合成选项
...
Result<std::vector<ValidationIssue>>
Validate(const Node& root,
         const std::vector<ValidationRule>&
             rules); // 验证配置节点，返回验证问题列表，如果没有问题则返回空列表，支持规则列表和失败快速选项
ValidationRule RequirePathRule(std::string path, bool fail_fast = false); // 验证规则：要求路径存在
ValidationRule ExpectKindRule(std::string path, NodeKind expected_kind,
                              bool fail_fast = false); // 验证规则：期望路径节点的类型
ValidationRule NumericRangeRule(std::string path, double min_value, double max_value,
                                bool fail_fast = false); // 验证规则：数值范围检查，适用于整数和双精度节点
ValidationRule
ChoiceRule(std::string path, std::vector<std::string> choices, bool case_insensitive = true,
...
ValidationRule DependencyRule(std::string path, std::string depends_on,
                              bool fail_fast = false); // 验证规则：依赖检查，确保如果路径存在则依赖路径也存在
ValidationRule
StringLengthRule(std::string path, std::size_t min_length, std::size_t max_length,
...
class PollReloader
{
  public:
    PollReloader(
        std::string base_file_path, std::string local_file_path = "",
    ...
    Result<ReloadEvent>
    Tick(std::uint64_t now_ms = 0); // 轮询检查配置文件更改，返回重新加载事件结果，支持传入当前时间戳以进行去抖动处理

    const Node* Current() const noexcept; // 获取当前有效配置节点的指针，如果没有有效配置则返回nullptr
    const std::vector<SourceAttribution>*
    CurrentSourceTrace() const noexcept;  // 获取当前配置的来源追踪列表的指针，如果没有有效配置则返回nullptr
    Result<Node> SnapshotCurrent() const; // 获取当前配置的快照，如果没有有效配置则返回错误
    Status ExportSnapshotToFile(std::string_view file_path, ConfigFormat preferred = ConfigFormat::Unknown,
    ...
    Status RestoreSnapshot(const Node& snapshot,
                           const std::vector<SourceAttribution>* source_trace =
                               nullptr); // 恢复到指定的配置快照和来源追踪，如果验证失败则返回错误状态
    Status
    RestoreSnapshotFromFile(std::string_view file_path, ConfigFormat preferred = ConfigFormat::Unknown,
...
ConfigFormat
DetectFormatFromPath(std::string_view file_path); // 从文件路径检测配置格式；Json/Ini 为稳定支持，Yaml/Toml 为实用子集。
Status RegisterParserAdapter(ParserAdapter adapter,
                             bool set_active = false); // 注册解析器适配器，如果set_active为true则同时设置为活动适配器
Status
UnregisterParserAdapter(std::string_view name); // 注销解析器适配器，根据名称进行注销，如果适配器不存在则返回错误状态
Status SetActiveParserAdapter(
    std::string_view
        name); // 设置活动的解析器适配器，根据名称进行设置，如果名称为空则清除活动适配器，如果适配器不存在则返回错误状态
...
const char* ToString(
    ConfigFormat format) noexcept; // 将配置格式枚举转换为字符串表示，返回对应的字符串常量，如果格式未知则返回"unknown"
const char*
ToString(NodeKind kind) noexcept; // 将节点类型枚举转换为字符串表示，返回对应的字符串常量，如果类型未知则返回"unknown"
const char*
ToString(SourceLayer layer) noexcept; // 将源层枚举转换为字符串表示，返回对应的字符串常量，如果层未知则返回"unknown"
const char*
ToString(DiffKind kind) noexcept; // 将差异类型枚举转换为字符串表示，返回对应的字符串常量，如果类型未知则返回"unknown"
std::optional<NodeKind> ParseNodeKind(
    std::string_view text); // 解析节点类型字符串为枚举值，支持多种别名和大小写不敏感，如果无法解析则返回std::nullopt

Result<Node> ParseJson(std::string_view text); // 解析JSON文本为节点，如果解析成功则返回节点，否则返回错误信息
std::string ToJson(const Node& node,
                   int indent = 2); // 将节点转换为JSON文本，支持指定缩进级别，如果indent小于0则不进行缩进

Status SetRemoteFetcher(RemoteFetcher fetcher); // 设置远程拉取回调，传入空回调表示清除。
bool HasRemoteFetcher();                        // 检查是否已设置远程拉取回调。
Result<Node> LoadFromRemote(std::string_view url, ConfigFormat preferred = ConfigFormat::Unknown,
                            HeaderList headers = {}); // 通过远程回调获取文本并解析为配置节点。

Result<Node> LoadFromFile(
    std::string_view file_path,
    ConfigFormat preferred = ConfigFormat::
        Unknown); // 从文件加载配置节点，支持指定首选格式，如果首选格式为Unknown则根据文件路径检测格式，如果加载或解析失败则返回错误信息
Status SaveToFile(
    const Node& root, std::string_view file_path, ConfigFormat preferred = ConfigFormat::Unknown,
    int indent =
        2); // 将配置节点保存到文件，支持指定首选格式和缩进级别，如果首选格式为Unknown则根据文件路径检测格式，如果保存或序列化失败则返回错误信息
Status SaveEncryptedToFile(const Node& root, std::string_view file_path, std::string_view key,
                           ConfigFormat preferred = ConfigFormat::Unknown,
                           int indent = 2); // 使用轻量本地保护格式写入配置文件；不提供认证加密安全保证。
Result<Node>
LoadEncryptedFromFile(std::string_view file_path, std::string_view key,
