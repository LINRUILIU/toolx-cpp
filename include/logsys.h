...
struct BackpressureConfigV2
{
    ...
    std::size_t queue_high_watermark{
        10000}; // 日志队列高水位线，当队列中的日志事件数量超过此值时，开始应用 backpressure 策略。
};
...
struct ProfileConfigV2
{
    ...
    std::optional<std::string>
        file_path_pattern;          // 可选的文件路径模式，支持简单的通配符（如 "src/*.cpp"）用于匹配日志事件来源文件。
...
struct LoggerConfigV2
{
    ...
    std::uint32_t global_text_field_mask{
    ...
    std::vector<ProfileConfigV2>
        profiles{}; // 配置档位列表，按优先级顺序排列，解析时会依次匹配文件路径和模块信息以确定最终配置。
};
...
{
  public:
    static ResolvedProfileV2 Resolve(const LoggerConfigV2& config, std::string_view file_path, ModuleId module);
};
// ProfileResolverV2: 根据文件路径与模块计算实际生效的配置（优先级由 module/file 决定）。

constexpr std::uint32_t kTextFieldMaskDefault =
    static_cast<std::uint32_t>(TextField::Timestamp) | static_cast<std::uint32_t>(TextField::Level) |
    static_cast<std::uint32_t>(TextField::Code) | static_cast<std::uint32_t>(TextField::Category) |
    static_cast<std::uint32_t>(TextField::Location) | static_cast<std::uint32_t>(TextField::Function) |
    static_cast<std::uint32_t>(TextField::Message) | static_cast<std::uint32_t>(TextField::SysCode) |
    static_cast<std::uint32_t>(TextField::Vendor) | static_cast<std::uint32_t>(TextField::ExtFields) |
...
{
    return (static_cast<ErrorCode>(source) << 24) | (static_cast<ErrorCode>(module) << 16) |
           static_cast<ErrorCode>(
               detail); // 构造错误代码的 constexpr 函数，将 source、module 和 detail 组合成一个 32 位整数。
}
...
ParseLogLevel(std::string_view level_text); // 将字符串解析为日志级别枚举的函数，返回 std::optional
                                            // 包含解析结果，如果解析失败则为 std::nullopt。
...
class ErrorDictionary
{
  public:
    ...
    std::optional<ErrorDictionaryEntry>
    Find(ErrorCode code) const; // 根据错误代码查找对应的错误字典条目的函数，返回 std::optional
                                // 包含查找结果，如果未找到则为 std::nullopt。
    const std::vector<ErrorDictionaryEntry>&
    Entries() const noexcept; // 获取错误字典中所有条目的函数，返回一个包含所有错误字典条目的常量引用。
...
class IFormatter
{
  public:
    ...
    virtual std::string Format(const LogEvent& event)
        const = 0; // 纯虚函数，定义日志事件格式化接口，接受一个 LogEvent 对象并返回格式化后的字符串。
};
...
class TextFormatter final : public IFormatter
{
  public:
    std::string Format(const LogEvent& event)
        const override; // 实现 IFormatter 接口的文本格式化器，重写 Format 函数以提供日志事件的文本格式化逻辑。
};
...
class JsonFormatter final : public IFormatter
{
  public:
    std::string Format(const LogEvent& event)
        const override; // 实现 IFormatter 接口的 JSON 格式化器，重写 Format 函数以提供日志事件的 JSON 格式化逻辑。
};
...
class FileSink final : public ISink
{
  public:
    explicit FileSink(std::string path,
                      RollingConfigV2 rolling = {}); // 构造函数，接受日志文件路径和可选的滚动配置参数。
...
class LevelRouter
{
  public:
    ...
    void AddLevelSink(LogLevel level, SinkPtr sink); // 添加级别特定输出器的函数，接受一个日志级别和一个 ISink
                                                     // 的智能指针，将该输出器添加到指定级别的输出器列表中。
    std::vector<SinkPtr> Resolve(LogLevel level)
        const; // 根据日志级别解析出对应的输出器列表的函数，接受一个日志级别参数，返回一个包含所有适用于该级别的输出器的列表。
...
class LogStreamBuilder
{
  public:
    LogStreamBuilder(
        Logger& logger, LogLevel level, ErrorCode code, ErrorCategory category, const char* file, std::uint32_t line,
        const char*
            function); // 构造函数，接受 Logger
                       // 引用、日志级别、错误代码、错误类别、文件路径、行号和函数名等参数，用于初始化日志事件构建器。
    ~LogStreamBuilder();

    template <typename T> LogStreamBuilder& operator<<(const T& v)
    {
        ss_ << v;
        return *this;
    ...
    LogStreamBuilder&
    SetField(std::string key,
             std::string value); // 设置扩展字段的成员函数，接受键值对参数，返回当前对象的引用以支持链式调用。
...
class Logger
{
  public:
    ...
    void SetFormatter(std::shared_ptr<IFormatter> formatter); // 设置日志格式化器的函数，接受一个 IFormatter
                                                              // 的智能指针，用于指定日志事件的格式化方式。
    void
    SetLevel(LogLevel level); // 设置输出级别阈值的函数，接受一个日志级别参数，控制哪些日志会被发送到输出端（sink）。
    LogLevel Level() const noexcept; // 获取当前输出级别阈值的函数，返回一个日志级别值。
    void SetRecordLevel(
        LogLevel level); // 设置记录级别阈值的函数，接受一个日志级别参数，控制哪些日志事件会被接受进入日志系统进行处理。
    ...
    void SetFlushOnFatal(bool enabled); // 设置是否在记录 Fatal
                                        // 级别日志时自动刷新输出的函数，接受一个布尔值参数，控制该行为的启用与禁用。
    ...
    // 设置默认错误来源、模块和类别，接受一个错误来源、一个模块 ID 和一个错误类别参数，
    // 用于指定默认的日志事件来源信息。
    void SetDefaultOrigin(ErrorSource source, ModuleId module, ErrorCategory category);
    ErrorCategory DefaultCategory() const noexcept; // 获取当前默认错误类别的函数，返回一个错误类别值。
    ErrorCode DefaultCodeForLevel(
        LogLevel level) const; // 根据日志级别获取默认错误代码的函数，接受一个日志级别参数，返回一个错误代码值。
    void ConfigureDefaultLogger(
        const DefaultLoggerOptions& options = {}); // 使用 DefaultLoggerOptions 配置默认 logger 的函数，接受一个
                                                   // DefaultLoggerOptions 结构体参数，提供便捷的默认配置选项。
    // 使用简单参数配置默认 logger，接受多个参数用于快速设置输出级别、记录级别和输出选项。
    void ConfigureSimpleLogger(LogLevel output_level = LogLevel::Fatal, bool enable_console = true,
                               bool enable_file = false, std::string file_path = "app.log",
                               LogLevel record_level = LogLevel::Info);
    void ApplyConfigV2(const LoggerConfigV2& config); // 应用完整 LoggerConfigV2 配置的函数，接受一个 LoggerConfigV2
                                                      // 结构体参数，提供更灵活和全面的配置选项。
    const LoggerConfigV2&
    CurrentConfigV2() const noexcept; // 获取当前生效的 LoggerConfigV2 配置的函数，返回一个常量引用指向当前配置。
    bool LoadConfigV2FromJsonFile(
        const std::string& file_path); // 从 JSON 文件加载 LoggerConfigV2
                                       // 配置的函数，接受一个文件路径参数，返回一个布尔值表示加载是否成功。
    bool SetLevelFromString(
        std::string_view
            level_text); // 从字符串设置输出级别的函数，接受一个字符串视图参数，返回一个布尔值表示解析和设置是否成功。
    bool SetLevelFromArgs(
        int argc, const char* const argv[],
        std::string_view key = "--log-level"); // 从命令行参数设置输出级别的函数，接受 argc 和 argv
                                               // 参数以及一个可选的键参数，返回一个布尔值表示解析和设置是否成功。
    void SetOutputEnabled(
        bool enable_console, bool enable_file,
        bool
            enable_debugger); // 同时设置多个输出选项的函数，接受三个布尔值参数，分别控制控制台、文件和调试器输出的启用与禁用。
    void
    EnableConsoleOutput(bool enabled);   // 设置是否启用控制台输出的函数，接受一个布尔值参数，控制该行为的启用与禁用。
    void EnableFileOutput(bool enabled); // 设置是否启用文件输出的函数，接受一个布尔值参数，控制该行为的启用与禁用。
    void
    EnableDebuggerOutput(bool enabled); // 设置是否启用调试器输出的函数，接受一个布尔值参数，控制该行为的启用与禁用。
    void SetTextFieldMask(
        std::uint32_t mask); // 设置文本字段掩码的函数，接受一个 32 位无符号整数参数，控制输出哪些字段（位掩码）。
    std::uint32_t TextFieldMask() const noexcept;            // 获取当前文本字段掩码，返回一个 32 位无符号整数。
    void SetTextFieldEnabled(TextField field, bool enabled); // 设置单个文本字段启用状态的函数，接受一个 TextField
                                                             // 枚举值和一个布尔值参数，控制该字段的启用与禁用。
    void SetAutoFillMissingMetadata(
        bool enabled); // 设置是否自动填充缺失元数据的函数，接受一个布尔值参数，控制该行为的启用与禁用。
    bool AutoFillMissingMetadata() const noexcept; // 获取当前是否自动填充缺失元数据的函数，返回一个布尔值。
    void StartPeriodicFlush(
        std::chrono::milliseconds interval); // 启动定期刷新日志输出的函数，接受一个时间间隔参数，控制刷新频率。
    ...
    void Logf(
        LogLevel level, ErrorCode code, ErrorCategory category, const char* file, std::uint32_t line,
        const char* function, std::uint32_t sys_code, const char* vendor_name, const char* vendor_code, const char* fmt,
        Args... args) // 格式化日志事件并记录的函数模板，接受日志级别、错误代码、错误类别、文件路径、行号、函数名、系统错误代码、供应商名称、供应商代码以及格式字符串和参数，构建日志事件并提交处理。
    {
...
#pragma warning(pop)
    ...
    template <typename... Args>
    void LogDefaultf(
        LogLevel level, const char* file, std::uint32_t line, const char* function, const char* fmt,
        Args... args) // 使用默认错误代码和类别格式化日志事件并记录的函数模板，接受日志级别、文件路径、行号、函数名以及格式字符串和参数，构建日志事件并提交处理，错误代码和类别由当前默认设置决定。
    {
    ...
    // 根据错误代码获取对应的错误事件数量，返回一个 64 位无符号整数表示该错误代码的事件数量。
    std::uint64_t GetErrorCountByCode(ErrorCode code) const;
    // 根据错误类别获取对应的错误事件数量，返回一个 64 位无符号整数表示该错误类别的事件数量。
    std::uint64_t GetErrorCountByCategory(ErrorCategory category) const;

    // Test-only API guarded by LOGSYS_ALLOW_TEST_API=1 environment variable.
    // Returns false when reset is blocked.
    bool ResetErrorCountersForTestOnly(); // 重置错误计数器的函数，仅供测试使用，受环境变量 LOGSYS_ALLOW_TEST_API=1
                                          // 控制，返回一个布尔值表示是否成功重置（当重置被阻止时返回 false）。
    void Flush();                         // 刷新所有输出器，确保所有待处理的日志事件都被写入输出端。
  ...
  private:
    ...
    struct CodeBucket
    {
       ...
       // 错误代码计数桶结构，包含一个原子变量存储错误代码（key）和一个原子变量存储该错误代码的事件数量（count）。
    ...
    void IncrementCounters(
        ErrorCode code,
        ErrorCategory
            category); // 增加错误代码和类别计数的私有函数，接受一个错误代码和一个错误类别参数，更新对应的计数器。
    ...
    static std::string Sanitize(
        std::string
            message); // 静态函数，清理日志消息内容以移除或转义不安全的字符，接受一个字符串参数，返回处理后的字符串。
    ...
    std::atomic<std::uint8_t> default_source_{
        static_cast<std::uint8_t>(ErrorSource::Business)}; // 默认错误来源，使用 uint8_t 存储以节省空间。
    std::atomic<std::uint8_t> default_module_{
        static_cast<std::uint8_t>(ModuleId::BusinessCommon)}; // 默认模块 ID，使用 uint8_t 存储以节省空间。
    std::atomic<std::uint8_t> default_category_{
        static_cast<std::uint8_t>(ErrorCategory::Business)}; // 默认错误类别，使用 uint8_t 存储以节省空间。
    ...
    std::array<std::atomic<std::uint64_t>, static_cast<std::size_t>(ErrorCategory::Count)>
        category_counts_{}; // 错误类别计数数组，使用固定大小的原子变量存储每个错误类别的事件数量，避免运行时的 map
                            // 分配。
...
// 简化错误代码的创建，接受错误来源、模块和细节参数，调用 MakeErrorCode 函数生成错误代码。
#define LOGSYS_MAKE_ERROR_CODE(source, module, detail)                                                                 \
    ::logsys::MakeErrorCode(static_cast<std::uint8_t>(source), static_cast<std::uint8_t>(module),                      \
                            static_cast<std::uint16_t>(detail))

#define LOGSYS_ERROR_SOURCE(code)                                                                                      \
    ::logsys::ErrorSourcePart(code) // 宏定义，从错误代码中提取错误来源部分，调用 ErrorSourcePart 函数。
#define LOGSYS_ERROR_MODULE(code)                                                                                      \
    ::logsys::ErrorModulePart(code) // 宏定义，从错误代码中提取错误模块部分，调用 ErrorModulePart 函数。
#define LOGSYS_ERROR_DETAIL(code)                                                                                      \
    ::logsys::ErrorDetailPart(code) // 宏定义，从错误代码中提取错误细节部分，调用 ErrorDetailPart 函数。

#define LOG_TRACE(code, category, fmt, ...)                                                                            \
    ::logsys::Logger::Instance().Logf(                                                                                 \
        ::logsys::LogLevel::Trace, code, category, __FILE__, static_cast<std::uint32_t>(__LINE__), __func__, 0, "",    \
        "", fmt, ##__VA_ARGS__) // 宏定义，记录 Trace 级别日志，接受错误代码、错误类别、格式字符串和参数，调用 Logger 的
                                // Logf 函数，并自动填充文件路径、行号和函数名等元数据。

#define LOG_DEBUG(code, category, fmt, ...)                                                                            \
    ::logsys::Logger::Instance().Logf(                                                                                 \
        ::logsys::LogLevel::Debug, code, category, __FILE__, static_cast<std::uint32_t>(__LINE__), __func__, 0, "",    \
        "", fmt, ##__VA_ARGS__) // 宏定义，记录 Debug 级别日志，接受错误代码、错误类别、格式字符串和参数，调用 Logger 的
                                // Logf 函数，并自动填充文件路径、行号和函数名等元数据。

#define LOG_INFO(code, category, fmt, ...)                                                                             \
    ::logsys::Logger::Instance().Logf(                                                                                 \
        ::logsys::LogLevel::Info, code, category, __FILE__, static_cast<std::uint32_t>(__LINE__), __func__, 0, "", "", \
        fmt, ##__VA_ARGS__) // 宏定义，记录 Info 级别日志，接受错误代码、错误类别、格式字符串和参数，调用 Logger 的 Logf
                            // 函数，并自动填充文件路径、行号和函数名等元数据。

#define LOG_WARNING(code, category, fmt, ...)                                                                          \
    ::logsys::Logger::Instance().Logf(                                                                                 \
        ::logsys::LogLevel::Warning, code, category, __FILE__, static_cast<std::uint32_t>(__LINE__), __func__, 0, "",  \
        "", fmt, ##__VA_ARGS__) // 宏定义，记录 Warning 级别日志，接受错误代码、错误类别、格式字符串和参数，调用 Logger
                                // 的 Logf 函数，并自动填充文件路径、行号和函数名等元数据。

#define LOG_ERROR(code, category, fmt, ...)                                                                            \
    ::logsys::Logger::Instance().Logf(                                                                                 \
        ::logsys::LogLevel::Error, code, category, __FILE__, static_cast<std::uint32_t>(__LINE__), __func__, 0, "",    \
        "", fmt, ##__VA_ARGS__) // 宏定义，记录 Error 级别日志，接受错误代码、错误类别、格式字符串和参数，调用 Logger 的
                                // Logf 函数，并自动填充文件路径、行号和函数名等元数据。

#define LOG_FATAL(code, category, fmt, ...)                                                                            \
    ::logsys::Logger::Instance().Logf(                                                                                 \
        ::logsys::LogLevel::Fatal, code, category, __FILE__, static_cast<std::uint32_t>(__LINE__), __func__, 0, "",    \
        "", fmt, ##__VA_ARGS__) // 宏定义，记录 Fatal 级别日志，接受错误代码、错误类别、格式字符串和参数，调用 Logger 的
                                // Logf 函数，并自动填充文件路径、行号和函数名等元数据。

#define LOG_CRITICAL(code, category, fmt, ...)                                                                         \
    ::logsys::Logger::Instance().Logf(                                                                                 \
        ::logsys::LogLevel::Critical, code, category, __FILE__, static_cast<std::uint32_t>(__LINE__), __func__, 0, "", \
        "", fmt, ##__VA_ARGS__) // 宏定义，记录 Critical 级别日志，接受错误代码、错误类别、格式字符串和参数，调用 Logger
                                // 的 Logf 函数，并自动填充文件路径、行号和函数名等元数据。

#define LOG_TRACE_STREAM(code, category)                                                                               \
    ::logsys::LogStreamBuilder(::logsys::Logger::Instance(), ::logsys::LogLevel::Trace, code, category, __FILE__,      \
                               static_cast<std::uint32_t>(__LINE__),                                                   \
                               __func__) // 宏定义，创建 Trace 级别日志流构建器，接受错误代码和错误类别参数，调用
                                         // LogStreamBuilder 构造函数，并自动填充文件路径、行号和函数名等元数据。

#define LOG_DEBUG_STREAM(code, category)                                                                               \
    ::logsys::LogStreamBuilder(::logsys::Logger::Instance(), ::logsys::LogLevel::Debug, code, category, __FILE__,      \
                               static_cast<std::uint32_t>(__LINE__),                                                   \
                               __func__) // 宏定义，创建 Debug 级别日志流构建器，接受错误代码和错误类别参数，调用
                                         // LogStreamBuilder 构造函数，并自动填充文件路径、行号和函数名等元数据。

#define LOG_INFO_STREAM(code, category)                                                                                \
    ::logsys::LogStreamBuilder(::logsys::Logger::Instance(), ::logsys::LogLevel::Info, code, category, __FILE__,       \
                               static_cast<std::uint32_t>(__LINE__),                                                   \
                               __func__) // 宏定义，创建 Info 级别日志流构建器，接受错误代码和错误类别参数，调用
                                         // LogStreamBuilder 构造函数，并自动填充文件路径、行号和函数名等元数据。

#define LOG_WARNING_STREAM(code, category)                                                                             \
    ::logsys::LogStreamBuilder(::logsys::Logger::Instance(), ::logsys::LogLevel::Warning, code, category, __FILE__,    \
                               static_cast<std::uint32_t>(__LINE__),                                                   \
                               __func__) // 宏定义，创建 Warning 级别日志流构建器，接受错误代码和错误类别参数，调用
                                         // LogStreamBuilder 构造函数，并自动填充文件路径、行号和函数名等元数据。

#define LOG_ERROR_STREAM(code, category)                                                                               \
    ::logsys::LogStreamBuilder(::logsys::Logger::Instance(), ::logsys::LogLevel::Error, code, category, __FILE__,      \
                               static_cast<std::uint32_t>(__LINE__),                                                   \
                               __func__) // 宏定义，创建 Error 级别日志流构建器，接受错误代码和错误类别参数，调用
                                         // LogStreamBuilder 构造函数，并自动填充文件路径、行号和函数名等元数据。

#define LOG_FATAL_STREAM(code, category)                                                                               \
    ::logsys::LogStreamBuilder(::logsys::Logger::Instance(), ::logsys::LogLevel::Fatal, code, category, __FILE__,      \
                               static_cast<std::uint32_t>(__LINE__),                                                   \
                               __func__) // 宏定义，创建 Fatal 级别日志流构建器，接受错误代码和错误类别参数，调用
                                         // LogStreamBuilder 构造函数，并自动填充文件路径、行号和函数名等元数据。

#define LOG_CRITICAL_STREAM(code, category)                                                                            \
    ::logsys::LogStreamBuilder(::logsys::Logger::Instance(), ::logsys::LogLevel::Critical, code, category, __FILE__,   \
                               static_cast<std::uint32_t>(__LINE__),                                                   \
                               __func__) // 宏定义，创建 Critical 级别日志流构建器，接受错误代码和错误类别参数，调用
                                         // LogStreamBuilder 构造函数，并自动填充文件路径、行号和函数名等元数据。

// Simple macros for small projects. They use default source/module/category.
#define LOGT(fmt, ...)                                                                                                 \
    ::logsys::Logger::Instance().LogDefaultf(                                                                          \
        ::logsys::LogLevel::Trace, __FILE__, static_cast<std::uint32_t>(__LINE__), __func__, fmt,                      \
        ##__VA_ARGS__) // 宏定义，记录 Trace 级别日志，接受格式字符串和参数，调用 Logger 的 LogDefaultf
                       // 函数，并自动填充文件路径、行号和函数名等元数据，使用默认错误代码和类别。

#define LOGD(fmt, ...)                                                                                                 \
    ::logsys::Logger::Instance().LogDefaultf(                                                                          \
        ::logsys::LogLevel::Debug, __FILE__, static_cast<std::uint32_t>(__LINE__), __func__, fmt,                      \
        ##__VA_ARGS__) // 宏定义，记录 Debug 级别日志，接受格式字符串和参数，调用 Logger 的 LogDefaultf
                       // 函数，并自动填充文件路径、行号和函数名等元数据，使用默认错误代码和类别。

#define LOGI(fmt, ...)                                                                                                 \
    ::logsys::Logger::Instance().LogDefaultf(                                                                          \
        ::logsys::LogLevel::Info, __FILE__, static_cast<std::uint32_t>(__LINE__), __func__, fmt,                       \
        ##__VA_ARGS__) // 宏定义，记录 Info 级别日志，接受格式字符串和参数，调用 Logger 的 LogDefaultf
                       // 函数，并自动填充文件路径、行号和函数名等元数据，使用默认错误代码和类别。

#define LOGW(fmt, ...)                                                                                                 \
    ::logsys::Logger::Instance().LogDefaultf(                                                                          \
        ::logsys::LogLevel::Warning, __FILE__, static_cast<std::uint32_t>(__LINE__), __func__, fmt,                    \
        ##__VA_ARGS__) // 宏定义，记录 Warning 级别日志，接受格式字符串和参数，调用 Logger 的 LogDefaultf
                       // 函数，并自动填充文件路径、行号和函数名等元数据，使用默认错误代码和类别。

#define LOGE(fmt, ...)                                                                                                 \
    ::logsys::Logger::Instance().LogDefaultf(                                                                          \
        ::logsys::LogLevel::Error, __FILE__, static_cast<std::uint32_t>(__LINE__), __func__, fmt,                      \
        ##__VA_ARGS__) // 宏定义，记录 Error 级别日志，接受格式字符串和参数，调用 Logger 的 LogDefaultf
                       // 函数，并自动填充文件路径、行号和函数名等元数据，使用默认错误代码和类别。

#define LOGF(fmt, ...)                                                                                                 \
    ::logsys::Logger::Instance().LogDefaultf(                                                                          \
        ::logsys::LogLevel::Fatal, __FILE__, static_cast<std::uint32_t>(__LINE__), __func__, fmt,                      \
        ##__VA_ARGS__) // 宏定义，记录 Fatal 级别日志，接受格式字符串和参数，调用 Logger 的 LogDefaultf
                       // 函数，并自动填充文件路径、行号和函数名等元数据，使用默认错误代码和类别。

#define LOGC(fmt, ...)                                                                                                 \
    ::logsys::Logger::Instance().LogDefaultf(                                                                          \
        ::logsys::LogLevel::Critical, __FILE__, static_cast<std::uint32_t>(__LINE__), __func__, fmt,                   \
        ##__VA_ARGS__) // 宏定义，记录 Critical 级别日志，接受格式字符串和参数，调用 Logger 的 LogDefaultf
                       // 函数，并自动填充文件路径、行号和函数名等元数据，使用默认错误代码和类别。

#define LOGT_STREAM()                                                                                                  \
    ::logsys::LogStreamBuilder(                                                                                        \
        ::logsys::Logger::Instance(), ::logsys::LogLevel::Trace,                                                       \
        ::logsys::Logger::Instance().DefaultCodeForLevel(::logsys::LogLevel::Trace),                                   \
        ::logsys::Logger::Instance().DefaultCategory(), __FILE__, static_cast<std::uint32_t>(__LINE__),                \
        __func__) // 宏定义，创建 Trace 级别日志流构建器，接受格式字符串和参数，调用 LogStreamBuilder
                  // 构造函数，并自动填充文件路径、行号和函数名等元数据，使用默认错误代码和类别。

#define LOGD_STREAM()                                                                                                  \
    ::logsys::LogStreamBuilder(                                                                                        \
        ::logsys::Logger::Instance(), ::logsys::LogLevel::Debug,                                                       \
        ::logsys::Logger::Instance().DefaultCodeForLevel(::logsys::LogLevel::Debug),                                   \
        ::logsys::Logger::Instance().DefaultCategory(), __FILE__, static_cast<std::uint32_t>(__LINE__),                \
        __func__) // 宏定义，创建 Debug 级别日志流构建器，接受格式字符串和参数，调用 LogStreamBuilder
                  // 构造函数，并自动填充文件路径、行号和函数名等元数据，使用默认错误代码和类别。

#define LOGI_STREAM()                                                                                                  \
    ::logsys::LogStreamBuilder(                                                                                        \
        ::logsys::Logger::Instance(), ::logsys::LogLevel::Info,                                                        \
        ::logsys::Logger::Instance().DefaultCodeForLevel(::logsys::LogLevel::Info),                                    \
        ::logsys::Logger::Instance().DefaultCategory(), __FILE__, static_cast<std::uint32_t>(__LINE__),                \
        __func__) // 宏定义，创建 Info 级别日志流构建器，接受格式字符串和参数，调用 LogStreamBuilder
                  // 构造函数，并自动填充文件路径、行号和函数名等元数据，使用默认错误代码和类别。

#define LOGW_STREAM()                                                                                                  \
    ::logsys::LogStreamBuilder(                                                                                        \
        ::logsys::Logger::Instance(), ::logsys::LogLevel::Warning,                                                     \
        ::logsys::Logger::Instance().DefaultCodeForLevel(::logsys::LogLevel::Warning),                                 \
        ::logsys::Logger::Instance().DefaultCategory(), __FILE__, static_cast<std::uint32_t>(__LINE__),                \
        __func__) // 宏定义，创建 Warning 级别日志流构建器，接受格式字符串和参数，调用 LogStreamBuilder
                  // 构造函数，并自动填充文件路径、行号和函数名等元数据，使用默认错误代码和类别。

#define LOGE_STREAM()                                                                                                  \
    ::logsys::LogStreamBuilder(                                                                                        \
        ::logsys::Logger::Instance(), ::logsys::LogLevel::Error,                                                       \
        ::logsys::Logger::Instance().DefaultCodeForLevel(::logsys::LogLevel::Error),                                   \
        ::logsys::Logger::Instance().DefaultCategory(), __FILE__, static_cast<std::uint32_t>(__LINE__),                \
        __func__) // 宏定义，创建 Error 级别日志流构建器，接受格式字符串和参数，调用 LogStreamBuilder
                  // 构造函数，并自动填充文件路径、行号和函数名等元数据，使用默认错误代码和类别。

#define LOGC_STREAM()                                                                                                  \
    ::logsys::LogStreamBuilder(                                                                                        \
        ::logsys::Logger::Instance(), ::logsys::LogLevel::Critical,                                                    \
        ::logsys::Logger::Instance().DefaultCodeForLevel(::logsys::LogLevel::Critical),                                \
        ::logsys::Logger::Instance().DefaultCategory(), __FILE__, static_cast<std::uint32_t>(__LINE__),                \
        __func__) // 宏定义，创建 Critical 级别日志流构建器，接受格式字符串和参数，调用 LogStreamBuilder
                  // 构造函数，并自动填充文件路径、行号和函数名等元数据，使用默认错误代码和类别。

// Lowercase convenience API for tiny projects.
#define log_info(fmt, ...)                                                                                             \
    LOGI(fmt, ##__VA_ARGS__) // 宏定义，记录 Info 级别日志的简化版本，接受格式字符串和参数，调用 LOGI 宏。
#define log_warn(fmt, ...)                                                                                             \
    LOGW(fmt, ##__VA_ARGS__) // 宏定义，记录 Warning 级别日志的简化版本，接受格式字符串和参数，调用 LOGW 宏。
#define log_error(fmt, ...)                                                                                            \
    LOGE(fmt, ##__VA_ARGS__) // 宏定义，记录 Error 级别日志的简化版本，接受格式字符串和参数，调用 LOGE 宏。
#define log_fatal(fmt, ...)                                                                                            \
    LOGF(fmt, ##__VA_ARGS__) // 宏定义，记录 Fatal 级别日志的简化版本，接受格式字符串和参数，调用 LOGF 宏。

// Stream-style aliases; use as: log_debug << "x=" << v << std::endl;
#define log_debug                                                                                                      \
    LOGD_STREAM() // 宏定义，创建 Debug 级别日志流构建器的简化版本，接受格式字符串和参数，调用 LOGD_STREAM 宏。
#define log_trace                                                                                                      \
    LOGT_STREAM() // 宏定义，创建 Trace 级别日志流构建器的简化版本，接受格式字符串和参数，调用 LOGT_STREAM 宏。
#define log_info_stream                                                                                                \
    LOGI_STREAM() // 宏定义，创建 Info 级别日志流构建器的简化版本，接受格式字符串和参数，调用 LOGI_STREAM 宏。
#define log_warn_stream                                                                                                \
    LOGW_STREAM() // 宏定义，创建 Warning 级别日志流构建器的简化版本，接受格式字符串和参数，调用 LOGW_STREAM 宏。
#define log_error_stream                                                                                               \
    LOGE_STREAM() // 宏定义，创建 Error 级别日志流构建器的简化版本，接受格式字符串和参数，调用 LOGE_STREAM 宏。
#define log_critical_stream                                                                                            \
    LOGC_STREAM() // 宏定义，创建 Critical 级别日志流构建器的简化版本，接受格式字符串和参数，调用 LOGC_STREAM 宏。
