...
template <typename T> using Result = sysx::Result<T>;

inline Error MakeError(ErrorKind kind, std::string message, ErrorDomain domain = ErrorDomain::System,
...
template <typename T> inline Result<T> FromSysx(const sysx::Result<T>& result)
{
    return result;
}

inline Status FromCfgx(const cfgx::Status& status, ErrorKind kind = ErrorKind::InvalidArgument,
...
template <typename T>
inline Result<T> FromCfgx(const cfgx::Result<T>& result, ErrorKind kind = ErrorKind::InvalidArgument,
                          ErrorDomain domain = ErrorDomain::System)
{
    if (result.ok)
    {
...
inline Status FromFsx(const fsx::Status& status, ErrorKind kind = ErrorKind::Internal,
                      ErrorDomain domain = ErrorDomain::System)
{
    if (status.ok)
    {
...
inline Result<fsx::RunResult> FromFsx(const fsx::RunResult& result, ErrorKind kind = ErrorKind::Internal,
                                      ErrorDomain domain = ErrorDomain::System)
{
    if (result.ok)
    {
...
inline Status FromAsyncx(const asyncx::Status& status)
{
    ...
    return Status{false, MakeError(MapAsyncxErrorKind(status.error.kind), status.error.message, ErrorDomain::System, 0,
                                   status.error.retryable)};
}

template <typename T> inline Result<T> FromAsyncx(const asyncx::Result<T>& result)
{
    if (result.ok)
    {
        return Result<T>{true, result.value, {}};
    ...
    return Result<T>{false,
                     {},
                     MakeError(MapAsyncxErrorKind(result.error.kind), result.error.message, ErrorDomain::System, 0,
...
inline Status FromHttpx(const httpx::Status& status)
{
    ...
    return Status{false, MakeError(MapHttpxErrorKind(status.error.kind), status.error.message, ErrorDomain::Network,
                                   status.error.http_status, status.error.retryable)};
}

template <typename T> inline Result<T> FromHttpx(const httpx::Result<T>& result)
{
    if (result.ok)
    {
        return Result<T>{true, result.value, {}};
    ...
    return Result<T>{false,
                     {},
                     MakeError(MapHttpxErrorKind(result.error.kind), result.error.message, ErrorDomain::Network,
                               result.error.http_status, result.error.retryable)};
}

inline std::string FormatError(const Error& error)
{
    std::string out = std::string(sysx::ToString(error.domain)) + ":" + sysx::ToString(error.kind);
...
template <typename T> inline Result<T> Propagate(Status status)
{
    if (status.ok)
    {
