...
template <typename Rep, typename Period> inline void SleepFor(std::chrono::duration<Rep, Period> duration)
{
    std::this_thread::sleep_for(duration);
}

template <typename Clock, typename Duration> inline void SleepUntil(std::chrono::time_point<Clock, Duration> deadline)
{
...
class Thread
{
  public:
    ...
    explicit Thread(Fn&& fn, Args&&... args) : impl_(std::forward<Fn>(fn), std::forward<Args>(args)...)
    {
    }
