...
说    明：
***************************************************************************/
void cct_showint(const int X, const int Y, const int num, const int bg_color, const int fg_color, const int rpt,const int wid)
{
	int i;
...
int cct_read_keyboard_and_mouse(int &MX, int &MY, int &MAction, int &keycode1, int &keycode2)
{
	...
	while (1) {
		...
		//ReadConsoleInput(__hin, &InputRec, 1, &res);
			...
			// 检查是否有输入事件但不阻塞
		if (PeekConsoleInput(__hin, &InputRec, 1, &res) && res > 0) {
			// 有事件，读取它
			ReadConsoleInput(__hin, &InputRec, 1, &res);
			// 处理事件...
		}
...
/***************************************************************************
函数名称：
功    能：清除输入缓冲区中的所有鼠标事件，避免延迟点击
输入参数：
返 回 值：返回清除的事件数量
说    明：在游戏状态切换时调用，防止之前的鼠标操作影响新的游戏状态
***************************************************************************/
int cct_flush_mouse_events(void)
{
	INPUT_RECORD InputRec;
	DWORD res;
	int flushed_count = 0;

	// 循环读取并丢弃所有鼠标事件，直到输入缓冲区为空
	while (PeekConsoleInput(__hin, &InputRec, 1, &res) && res > 0) {
		// 读取事件
		ReadConsoleInput(__hin, &InputRec, 1, &res);
