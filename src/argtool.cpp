...
std::string FormatOptionNames(const std::string& long_name, char short_name,
                              const std::vector<std::string>& long_aliases, const std::vector<char>& short_aliases)
{
...
Parser::OptionBuilder Parser::Option(std::string long_name, char short_name)
{
    ...
    {
        if (existing.long_name == long_name || std::find(existing.long_aliases.begin(), existing.long_aliases.end(),
                                                         long_name) != existing.long_aliases.end())
        {
            throw std::invalid_argument("Duplicate option long_name: '--" + long_name + "'.");
        }
        if (short_name != '\0' && (existing.short_name == short_name ||
                                   std::find(existing.short_aliases.begin(), existing.short_aliases.end(),
                                             short_name) != existing.short_aliases.end()))
        {
...
Parser& Parser::AddOptionTemplate(const OptionTemplate& tpl)
{
    ...
    builder.Required(tpl.required).Repeat(tpl.repeat_mode).Description(tpl.description).Choices(tpl.choices);
...
Parser& Parser::AddPositionalTemplate(const PositionalTemplate& tpl)
{
    ...
    builder.Required(tpl.required).Variadic(tpl.variadic).Description(tpl.description).Choices(tpl.choices);
...
Parser& Parser::AddDependency(DependencyRule rule)
{
    ...
    if (!right_found)
    {
        throw std::invalid_argument("DependencyRule references unknown required option: '--" +
                                    rule.required_option_name + "'.");
    }
...
void Parser::ValidateOptionSpecOrThrow(const OptionSpec& spec, std::size_t index) const
{
    ...
    for (const auto& raw_alias : spec.long_aliases)
    {
        ...
        if (std::find(normalized_long_aliases.begin(), normalized_long_aliases.end(), alias) !=
            normalized_long_aliases.end())
        {
    ...
    for (char alias : spec.short_aliases)
    {
        ...
        if (std::find(normalized_short_aliases.begin(), normalized_short_aliases.end(), alias) !=
            normalized_short_aliases.end())
        {
    ...
    if (spec.range_policy == RangePolicy::UseDefaultAndWarn &&
        (spec.min_value.has_value() || spec.max_value.has_value()) && !spec.default_value.has_value())
    {
...
void Parser::ValidatePositionalSpecOrThrow(const PositionalSpec& spec, std::size_t index) const
{
    ...
    if (spec.range_policy == RangePolicy::UseDefaultAndWarn &&
        (spec.min_value.has_value() || spec.max_value.has_value()) && !spec.default_value.has_value())
    {
        throw std::invalid_argument("RangePolicy::UseDefaultAndWarn requires default value for positional '" +
                                    spec.name + "'.");
    }
...
ParseResult Parser::Fail(ParseResult result, ParseErrorKind kind, std::string field, std::string token,
...
ParseResult Parser::ApplyConstraintPipeline(ParseResult result) const
{
    ...
    for (std::size_t i = 0; i < dependency_rules_.size(); ++i)
    {
        ...
        rule.evaluator = [dep](const ConstraintContext& context)
        {
            ...
            out.error.message = !dep.message.empty() ? dep.message
                                                     : "Option '--" + dep.option_name + "' requires '--" +
                                                           dep.required_option_name + "'.";
            return out;
        };
        runtime_rules.push_back(std::move(rule));
    }
    ...
    std::stable_sort(runtime_rules.begin(), runtime_rules.end(),
                     [](const ConstraintRule& lhs, const ConstraintRule& rhs)
                     {
    ...
    for (const auto& rule : runtime_rules)
    {
        ...
        if (rule.fail_fast)
        {
            return Fail(std::move(result), evaluated.error.kind, evaluated.error.field, evaluated.error.token,
    ...
    if (deferred_error.has_value())
    {
        return Fail(std::move(result), deferred_error->kind, deferred_error->field, deferred_error->token,
...
ParseResult Parser::Parse(int argc, const char* const argv[]) const
{
    ...
    const std::string effective_program_name =
        !program_name_.empty() ? program_name_
                               : (argc > 0 && argv[0] != nullptr ? std::string(argv[0]) : std::string("program"));
    ...
    auto validate_typed_value = [&](std::string value, ValueType type, std::string_view field, std::string_view token,
                                    std::optional<double> min_value, std::optional<double> max_value,
                                    const std::vector<std::string>& choices, RangePolicy range_policy,
                                    const std::optional<std::string>& default_value,
                                    const std::optional<ValueConverter>& local_converter, std::string* normalized,
                                    ParseResult current_result) -> std::optional<ParseResult>
    {
        ...
        else
        {
            ...
            {
            case ValueType::String:
                converter = global_string_converter_.has_value() ? *global_string_converter_
                                                                 : ValueConverter(BuiltinStringConverter);
                break;
            case ValueType::Int:
                converter =
                    global_int_converter_.has_value() ? *global_int_converter_ : ValueConverter(BuiltinIntConverter);
                break;
            case ValueType::Double:
                converter = global_double_converter_.has_value() ? *global_double_converter_
                                                                 : ValueConverter(BuiltinDoubleConverter);
                break;
            case ValueType::Bool:
                converter =
                    global_bool_converter_.has_value() ? *global_bool_converter_ : ValueConverter(BuiltinBoolConverter);
                break;
            }
        }
        ...
        if (!choices.empty())
        {
            ...
            {
                return Fail(std::move(current_result), ParseErrorKind::ChoiceError, std::string(field),
                            std::string(token),
        ...
        auto fail_range = [&](double numeric_value) -> std::optional<ParseResult>
        {
            ...
            return Fail(std::move(current_result), ParseErrorKind::RangeError, std::string(field), std::string(token),
        ...
        if (type == ValueType::Int)
        {
            ...
            {
                return Fail(std::move(current_result), ParseErrorKind::TypeMismatch, std::string(field),
                            std::string(token), "Invalid int value: '" + value + "'.");
            }
        ...
        if (type == ValueType::Double)
        {
            ...
            {
                return Fail(std::move(current_result), ParseErrorKind::TypeMismatch, std::string(field),
                            std::string(token), "Invalid double value: '" + value + "'.");
            }
        ...
        return Fail(std::move(current_result), ParseErrorKind::InternalError, std::string(field), std::string(token),
    ...
    for (int i = start_index; i < argc; ++i)
    {
        ...
        if (token.size() > 2 && token.rfind("--", 0) == 0)
        {
            ...
            if (option == nullptr)
            {
                ...
                return Fail(result, ParseErrorKind::UnknownOption, long_name, token,
                            "Unknown option: '--" + long_name + "'.");
            }
            ...
            if (!option->takes_value)
            {
                ...
                {
                    return Fail(result, ParseErrorKind::TypeMismatch, option->long_name, token,
            ...
            else
            {
                if (i + 1 >= argc)
                {
                    return Fail(result, ParseErrorKind::MissingValue, option->long_name, token,
            ...
            if (auto err =
                    validate_typed_value(raw_value, option->value_type, option->long_name, token, option->min_value,
                                         option->max_value, option->choices, option->range_policy,
                                         option->default_value, option->converter, &normalized, result);
                err.has_value())
            {
                return *err;
            }
        ...
        if (token.size() >= 2 && token[0] == '-' && token[1] != '-')
        {
            if (token.size() == 2)
            {
                ...
                if (option == nullptr)
                {
                    if (unknown_option_handler_)
                    {
                        ...
                        if (!callback_error.empty())
                        {
                            return Fail(result, ParseErrorKind::UnknownOption, std::string(1, short_name), token,
                                        callback_error);
                        }
                    }
                    return Fail(result, ParseErrorKind::UnknownOption, std::string(1, short_name), token,
            ...
            if (!all_flags)
            {
                return Fail(result, ParseErrorKind::TypeMismatch, token, token,
    ...
    for (const auto& positional : positionals_)
    {
        if (positional.variadic)
        {
            ...
            {
                return Fail(result, ParseErrorKind::MissingRequired, positional.name, positional.name,
    ...
    if (token_index < positional_tokens.size())
    {
        return Fail(result, ParseErrorKind::UnexpectedPositional, "positional", positional_tokens[token_index],
    ...
    for (const auto& option : options_)
    {
        ...
        {
            return Fail(result, ParseErrorKind::MissingRequired, option.long_name, "--" + option.long_name,
...
{
    const std::string effective_program_name =
        !program_name_.empty() ? program_name_ : (!runtime_program_name_.empty() ? runtime_program_name_ : "program");
    ...
    if (layout == HelpLayout::Compact)
    {
        ...
        for (const auto& option : options_)
        {
            const std::string names =
                FormatOptionNames(option.long_name, option.short_name, option.long_aliases, option.short_aliases);
            if (option.takes_value)
            {
    ...
    auto dash = [](std::size_t n) { return std::string(n, '-'); };
    ...
    for (const auto& option : options_)
    {
        ...
        if (!option.takes_value)
        {
            flag_rows.push_back({names, option.required ? "yes" : "no",
                                 option.default_value.has_value() ? *option.default_value : "none",
                                 BoolFlagModeName(option.bool_mode), constraints_text,
        ...
        else
        {
            ...
            value_rows.push_back({names_with_value, ValueTypeName(option.value_type), option.required ? "yes" : "no",
                                  option.default_value.has_value() ? *option.default_value : "none",
                                  RepeatModeName(option.repeat_mode), constraints_text,
                                  option.description.empty() ? "none" : option.description});
        }
    }
    ...
    for (const auto& positional : positionals_)
    {
        ...
        positional_rows.push_back({positional.variadic ? positional.name + "..." : positional.name,
                                   ValueTypeName(positional.value_type), positional.required ? "yes" : "no",
    ...
    auto print_seven_col = [&](std::string_view title, std::string_view c1, std::string_view c2, std::string_view c3,
                               std::string_view c4, std::string_view c5, std::string_view c6,
                               const std::vector<std::array<std::string, 6>>& rows)
    {
        ...
        oss << "\n" << title << ":\n";
        oss << "  " << std::left << std::setw(static_cast<int>(w1)) << c1 << ' ' << std::setw(static_cast<int>(w2))
            << c2 << ' ' << std::setw(static_cast<int>(w3)) << c3 << ' ' << std::setw(static_cast<int>(w4)) << c4 << ' '
            << std::setw(static_cast<int>(w5)) << c5 << ' ' << c6 << "\n";
        oss << "  " << dash(w1) << ' ' << dash(w2) << ' ' << dash(w3) << ' ' << dash(w4) << ' ' << dash(w5) << ' '
            << dash(std::max<std::size_t>(w6, 20)) << "\n";
        ...
        for (const auto& r : rows)
        {
            ...
            oss << "  " << std::left << std::setw(static_cast<int>(w1)) << r[0] << ' '
                << std::setw(static_cast<int>(w2)) << r[1] << ' ' << std::setw(static_cast<int>(w3)) << r[2] << ' '
                << std::setw(static_cast<int>(w4)) << r[3] << ' ' << std::setw(static_cast<int>(w5)) << r[4] << ' '
                << wrapped.front() << "\n";
            for (std::size_t i = 1; i < wrapped.size(); ++i)
            {
                oss << "  " << std::setw(static_cast<int>(w1)) << "" << ' ' << std::setw(static_cast<int>(w2)) << ""
                    << ' ' << std::setw(static_cast<int>(w3)) << "" << ' ' << std::setw(static_cast<int>(w4)) << ""
                    << ' ' << std::setw(static_cast<int>(w5)) << "" << ' ' << wrapped[i] << "\n";
            }
        }
    };
...

    {
        ...
        print_seven_col("Value Options", "Options", "Type", "Required", "Default", "Repeat",
                        "Description [Constraints]", rows);
    }

    {
        ...
        print_seven_col("Positional Arguments", "Name", "Type", "Required", "Default", "Arity",
                        "Description [Constraints]", rows);
    }
    ...
    if (!mutex_groups_.empty() || !dependency_rules_.empty())
    {
        ...
        {
            oss << "  Dependencies:\n";
            oss << "    " << std::left << std::setw(20) << "If Present" << ' ' << std::setw(20) << "Requires" << ' '
                << "Message" << "\n";
            oss << "    " << dash(20) << ' ' << dash(20) << ' ' << dash(40) << "\n";
            for (const auto& rule : dependency_rules_)
            {
                oss << "    " << std::left << std::setw(20) << ("--" + rule.option_name) << ' ' << std::setw(20)
                    << ("--" + rule.required_option_name) << ' ' << (rule.message.empty() ? "none" : rule.message)
                    << "\n";
            }
        }
    }
    ...
    if (!sub_names.empty())
    {
        ...
        oss << "\nSubcommands:\n";
        oss << "  " << std::left << std::setw(static_cast<int>(w_name)) << "Name" << ' ' << "Description" << "\n";
