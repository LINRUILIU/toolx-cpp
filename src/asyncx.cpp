...
Status ThreadPool::Start()
{
    ...
    try
    {
        ...
        scheduler_thread_ = sysx::thread::Thread([this]() { SchedulerLoop(); });
    }
    catch (...)
    {
        {
...
{
    {
        ...
        if (!scheduled_tasks_.empty())
        {
            ...
            scheduled_cancelled_.fetch_add(static_cast<std::uint64_t>(scheduled_tasks_.size()),
...
Status ThreadPool::PostWithPriorityUntil(std::chrono::steady_clock::time_point deadline, TaskPriority priority,
...
Result<std::uint64_t> ThreadPool::ScheduleEvery(std::chrono::steady_clock::duration interval,
                                                std::function<void()> task, bool run_immediately)
{
    Result<std::uint64_t> out;
    if (!task)
    {
...
Status ThreadPool::EnqueueTask(Task task, OptionalDeadline deadline, bool wait_for_slot, TaskPriority priority)
{
    if (!task)
    {
        rejected_.fetch_add(1, std::memory_order_relaxed);
    ...
    auto has_room = [this]() { return queue_.size() < options_.queue_capacity; };
    ...
    if (!has_room())
    {
        ...
        if (deadline.has_value())
        {
            while (!has_room())
            {
                ...
                {
                    rejected_.fetch_add(1, std::memory_order_relaxed);
                    return MakeErrorStatus(ErrorKind::QueueClosed, "thread pool stopped while waiting for queue slot",
                                           true);
                }
        ...
        {
            cv_not_full_.wait(lock, [this, &has_room]() { return has_room() || !running_ || !accepting_; });
...
Status ThreadPool::WaitForIdleImpl(OptionalDeadline deadline)
{
    ...
    if (!deadline.has_value())
    {
        cv_idle_.wait(lock, [this]() { return IsIdleLocked(); });
        return OkStatus();
    }

    if (!cv_idle_.wait_until(lock, *deadline, [this]() { return IsIdleLocked(); }))
    {
        timed_out_.fetch_add(1, std::memory_order_relaxed);
...
void ThreadPool::SchedulerLoop()
{
    for (;;)
    {
        ...
        const Status dispatch_status = EnqueueTask(std::move(dispatch_task), std::nullopt, true, TaskPriority::Normal);
...
void ThreadPool::WorkerLoop()
{
    constexpr TaskPriority kDispatchSlots[] = {
        ...
        TaskPriority::Normal, TaskPriority::Normal, TaskPriority::Low,
    };
    ...
    for (;;)
    {
        ...
        {
            std::unique_lock<sysx::sync::Mutex> lock(mu_);
            cv_not_empty_.wait(lock, [this]() { return stop_requested_ || !queue_.empty(); });
