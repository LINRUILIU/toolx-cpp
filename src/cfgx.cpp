...
#ifdef _WIN32
extern "C" __declspec(dllimport) char* __stdcall GetEnvironmentStringsA(
    void); // Windows API函数，获取环境变量字符串，返回指向环境变量块的指针，每个环境变量以null结尾，整个块以双null结尾
...
Node ParseScalar(std::string_view raw, bool strict_string = false)
{
    ...
    {
        const bool finite =
            (double_value <= std::numeric_limits<double>::max() && double_value >= -std::numeric_limits<double>::max());
        if (finite)
        {
...
bool ResolveStrictStringPolicy(const TypePolicyOptions& policy, SourceLayer layer, std::string_view path)
{
...
std::optional<std::string> ParseEnvPath(std::string_view key, std::string_view prefix, std::string& error)
{
...
bool AcquireActiveAdapter(std::function<Result<Node>(std::string_view, ConfigFormat)>* parse,
                          std::function<Result<std::string>(const Node&, ConfigFormat, int)>* dump, std::string* name)
{
...
bool IsScalarNode(const Node& node)
{
    ...
    return kind == NodeKind::Null || kind == NodeKind::Bool || kind == NodeKind::Integer || kind == NodeKind::Double ||
...
Result<FileFingerprint> CaptureFileFingerprint(std::string_view path, bool allow_missing)
{
    ...
    if (ec)
    {
        return Result<FileFingerprint>{
            false, {}, "filesystem exists() failed for '" + std::string(path) + "': " + ec.message()};
    }
    ...
    {
        return Result<FileFingerprint>{
            false, {}, "last_write_time failed for '" + std::string(path) + "': " + ec.message()};
    }
...
void FlattenLeafNodes(const Node& node, const std::string& path, std::map<std::string, Node>& out)
{
    if (node.IsObject())
    {
...
std::vector<DiffEntry> BuildDiffEntries(const Node* old_root, const Node& new_root,
                                        const std::vector<SourceAttribution>& trace)
{
    ...
    for (const auto& entry : new_flat)
    {
        ...
        if (old_it == old_flat.end())
        {
            ...
            diff.push_back(DiffEntry{entry.first, DiffKind::Added,
                                     layer_it == trace_map.end() ? SourceLayer::Base : layer_it->second});
            continue;
        }
        ...
        {
            const auto layer_it = trace_map.find(entry.first);
            diff.push_back(DiffEntry{entry.first, DiffKind::Changed,
                                     layer_it == trace_map.end() ? SourceLayer::Base : layer_it->second});
        }
    }
    ...
    std::sort(diff.begin(), diff.end(), [](const DiffEntry& lhs, const DiffEntry& rhs) { return lhs.path < rhs.path; });
    return diff;
}
...
Node::Node() : data_(std::monostate{}) {}

Node::Node(bool value) : data_(value) {}

Node::Node(std::int64_t value) : data_(value) {}

Node::Node(double value) : data_(value) {}

Node::Node(std::string value) : data_(std::move(value)) {}

Node::Node(const char* value) : data_(std::string(value != nullptr ? value : "")) {}

Node::Node(Object value) : data_(std::move(value)) {}
...
Result<const Node*> GetNode(const Node& root, std::string_view path)
{
    ...
    for (const auto& token : parsed.value)
    {
        if (token.kind == PathTokenKind::Key)
        {
            ...
            if (obj == nullptr)
            {
                return Result<const Node*>{false, nullptr,
                                           "type mismatch: expected object while traversing key '" + token.key + "'"};
            }
...
Result<Node> BuildEnvLayerFromPairs(const std::vector<std::pair<std::string, std::string>>& variables,
                                    std::string_view prefix, const TypePolicyOptions& policy)
{
    if (prefix.empty())
    {
...
Result<Node> BuildEnvLayerFromEnvironment(std::string_view prefix, const TypePolicyOptions& policy)
{
    const auto variables = CollectEnvironmentPairs();
    return BuildEnvLayerFromPairs(variables, prefix, policy);
}

Result<Node> ComposeLayers(const Node& base, const std::optional<Node>& env_layer,
                           const std::optional<Node>& local_layer, const RuntimeOverrides* runtime,
                           const ComposeOptions& options, std::vector<SourceAttribution>* source_trace,
                           const std::optional<Node>& remote_layer)
{
    ...
    if (source_trace != nullptr)
    {
        ...
        std::sort(source_trace->begin(), source_trace->end(),
                  [](const SourceAttribution& lhs, const SourceAttribution& rhs) { return lhs.path < rhs.path; });
    }

    return Result<Node>{true, std::move(out), ""};
}

PollReloader::PollReloader(std::string base_file_path, std::string local_file_path, const RuntimeOverrides* runtime)
    : base_file_path_(std::move(base_file_path)), local_file_path_(std::move(local_file_path)), runtime_(runtime)
{
}
...
Status PollReloader::ExportSnapshotToFile(std::string_view file_path, ConfigFormat preferred, int indent) const
{
    if (!has_current_)
    {
...
Result<Node> PollReloader::ImportSnapshotFromFile(std::string_view file_path, ConfigFormat preferred) const
{
    return LoadFromFile(file_path, preferred);
}

Status PollReloader::RestoreSnapshot(const Node& snapshot, const std::vector<SourceAttribution>* source_trace)
{
    current_ = snapshot;
    has_current_ = true;
...
Status PollReloader::RestoreSnapshotFromFile(std::string_view file_path, ConfigFormat preferred,
...
Result<ReloadEvent> PollReloader::ReloadNow()
{
    ...
    const auto composed =
        ComposeLayers(base, env_layer.value, local_layer, runtime_, options_.compose_options, &trace, remote_layer);
    if (!composed.ok)
    {
...
Result<ReloadEvent> PollReloader::Tick(std::uint64_t now_ms)
{
    ...
    const bool remote_poll_enabled = !TrimCopy(options_.remote_url).empty() && options_.remote_poll_interval_ms > 0;
    ...
    auto read_state = [&](std::string_view path, bool allow_missing,
                          TrackedFileState& tracked) -> Result<TrackedFileState>
    {
        const auto fp = CaptureFileFingerprint(path, allow_missing);
        if (!fp.ok)
        {
    ...
    const bool changed =
        state_changed(base_state_, base_observed.value) || state_changed(local_state_, local_observed.value);
...
ValidationRule ExpectKindRule(std::string path, NodeKind expected_kind, bool fail_fast)
{
    ...
    rule.evaluator = [path = std::move(path), expected_kind](const Node& root) -> std::optional<ValidationIssue>
    {
        ...
        ValidationIssue issue;
        issue.path = path;
        issue.message = std::string("expected kind '") + ToString(expected_kind) + "' but got '" +
                        ToString(node.value->Kind()) + "'";
        return issue;
    };
    return rule;
}

ValidationRule NumericRangeRule(std::string path, double min_value, double max_value, bool fail_fast)
{
    ValidationRule rule;
    ...
    rule.evaluator = [path = std::move(path), min_value, max_value](const Node& root) -> std::optional<ValidationIssue>
    {
        ...
        {
            std::ostringstream oss;
            oss << "numeric value " << value << " is out of range [" << min_value << "," << max_value << "]";
...
ValidationRule ChoiceRule(std::string path, std::vector<std::string> choices, bool case_insensitive, bool fail_fast)
{
    ValidationRule rule;
    rule.name = "choice";
    rule.fail_fast = fail_fast;
    rule.evaluator = [path = std::move(path), choices = std::move(choices),
                      case_insensitive](const Node& root) -> std::optional<ValidationIssue>
    {
        if (choices.empty())
        {
...
ValidationRule MutexRule(std::vector<std::string> paths, bool fail_fast)
{
    ValidationRule rule;
    rule.name = "mutex";
    rule.fail_fast = fail_fast;
...
ValidationRule DependencyRule(std::string path, std::string depends_on, bool fail_fast)
{
    ValidationRule rule;
    rule.name = "dependency";
    rule.fail_fast = fail_fast;
    rule.evaluator = [path = std::move(path),
                      depends_on = std::move(depends_on)](const Node& root) -> std::optional<ValidationIssue>
    {
        if (!Exists(root, path))
        {
            return std::nullopt;
...
ValidationRule StringLengthRule(std::string path, std::size_t min_length, std::size_t max_length, bool fail_fast)
{
    ValidationRule rule;
    rule.name = "string-length";
    rule.fail_fast = fail_fast;
    rule.evaluator = [path = std::move(path), min_length,
                      max_length](const Node& root) -> std::optional<ValidationIssue>
    {
...
Result<Node> LoadFromRemote(std::string_view url, ConfigFormat preferred, HeaderList headers)
{
    ...
    const ConfigFormat format =
        (preferred == ConfigFormat::Unknown) ? DetectFormatFromPath(StripUrlQueryAndFragment(url)) : preferred;
        ...
        if (sequence_mode)
        {
            ...
            while (index < lines.size())
            {
                ...
                const std::string item_text =
                    line_text.size() <= 1 ? "" : TrimCopy(std::string_view(line_text).substr(2));
                ++index;
                ...
                if (colon != std::string::npos)
                {
                    ...
                    if (!key.empty())
                    {
                        ...
                        if (index < lines.size() && lines[index].indent > indent)
                        {
                            ...
                            {
                                std::ostringstream err;
                                err << "expected mapping continuation for list item at line "
                                    << lines[index - 1].line_no;
                                return Result<Node>{false, Node{}, err.str()};
                            }
...
{
    return {Fnv1a32(key_text, 2166136261u), Fnv1a32(key_text, 2166136261u ^ 0x9e3779b9u),
            Fnv1a32(key_text, 2166136261u ^ 0x243f6a88u), Fnv1a32(key_text, 2166136261u ^ 0xb7e15162u)};
}
...
void ApplyTeaCtrXor(std::vector<std::uint8_t>& buffer, const std::array<std::uint32_t, 4>& key, std::uint64_t nonce)
{
    std::uint64_t counter = 0;
    for (std::size_t offset = 0; offset < buffer.size(); offset += 8)
    {
        std::uint32_t v0 =
            static_cast<std::uint32_t>((nonce >> 32) & 0xFFFFFFFFu) ^ static_cast<std::uint32_t>(counter & 0xFFFFFFFFu);
        std::uint32_t v1 =
            static_cast<std::uint32_t>(nonce & 0xFFFFFFFFu) ^ static_cast<std::uint32_t>((counter >> 32) & 0xFFFFFFFFu);
        TeaEncryptBlock(v0, v1, key);

        std::array<std::uint8_t, 8> keystream = {
            static_cast<std::uint8_t>((v0 >> 24) & 0xFFu), static_cast<std::uint8_t>((v0 >> 16) & 0xFFu),
            static_cast<std::uint8_t>((v0 >> 8) & 0xFFu),  static_cast<std::uint8_t>(v0 & 0xFFu),
            static_cast<std::uint8_t>((v1 >> 24) & 0xFFu), static_cast<std::uint8_t>((v1 >> 16) & 0xFFu),
            static_cast<std::uint8_t>((v1 >> 8) & 0xFFu),  static_cast<std::uint8_t>(v1 & 0xFFu)};
...
Result<Node> ParseTextByFormat(std::string_view text, ConfigFormat format)
{
    ...
    return Result<Node>{false, Node{},
                        "unsupported config format in parser dispatch: " + std::string(ToString(format))};
}
...
Result<std::string> RenderTextByFormat(const Node& root, ConfigFormat format, int indent)
{
    ...
    return Result<std::string>{false, "",
                               "unsupported config format in render dispatch: " + std::string(ToString(format))};
}
...
Status SaveEncryptedToFile(const Node& root, std::string_view file_path, std::string_view key, ConfigFormat preferred,
                           int indent)
{
...
Result<Node> LoadEncryptedFromFile(std::string_view file_path, std::string_view key, ConfigFormat preferred)
{
    if (TrimCopy(key).empty())
    {
