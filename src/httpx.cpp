...
bool ParseWindowsAddressLiteral(int family, const std::string& text, void* addr)
{
    ...
    if (WSAStringToAddressA(mutable_text.data(), family, nullptr, reinterpret_cast<sockaddr*>(&storage),
                            &storage_size) != 0)
    {
        return false;
    }
...
{
  public:
    explicit ScopedSocket(SocketHandle handle) : handle_(handle) {}
...
bool IsSensitiveQueryKey(std::string_view key)
{
    ...
    return lowered == "token" || lowered == "access_token" || lowered == "auth" || lowered == "password" ||
           lowered == "sig" || lowered == "signature";
}
...
void RemoveHeader(httpx::HeaderList* headers, std::string_view key)
{
    ...
    headers->erase(std::remove_if(headers->begin(), headers->end(),
                                  [&needle](const auto& kv) { return ToLower(kv.first) == needle; }),
                   headers->end());
}
...
void PruneExpiredCookies(std::vector<CookieEntry>* jar)
{
    ...
    jar->erase(std::remove_if(jar->begin(), jar->end(), [now](const CookieEntry& cookie)
...
void ReleasePooledSocket(const httpx::Client* owner, const std::string& key, SocketHandle socket,
...
WaitState WaitSocketReady(SocketHandle socket, bool want_read, std::uint64_t timeout_ms, int* error_code)
{
...
httpx::Result<std::vector<Endpoint>> ResolveEndpoints(const ParsedUrl& url, httpx::DnsStrategy dns_strategy)
{
    ...
    {
#if defined(_WIN32)
        out.error = MakeError(httpx::ErrorKind::Network, "getaddrinfo failed: " + std::to_string(rc), true);
#else
        out.error = MakeError(httpx::ErrorKind::Network, std::string("getaddrinfo failed: ") + gai_strerror(rc), true);
#endif
        return out;
    }
...
httpx::Result<SocketHandle> ConnectEndpoint(const Endpoint& endpoint, std::uint64_t connect_timeout_ms)
{
    ...
    {
        out.error =
            MakeError(httpx::ErrorKind::Network, "socket create failed: " + SocketErrorText(LastSocketError()), true);
        return out;
    }
    ...
    if (!SetBlockingMode(socket, false))
    {
        out.error = MakeError(httpx::ErrorKind::Network,
                              "set non-blocking failed: " + SocketErrorText(LastSocketError()), true);
        return out;
    }

    const int rc = ::connect(socket, reinterpret_cast<const sockaddr*>(&endpoint.address), endpoint.length);
    if (rc != 0)
    {
        const int connect_error = LastSocketError();
        if (!IsWouldBlockError(connect_error))
        {
            out.error = MakeError(httpx::ErrorKind::Network, "connect failed: " + SocketErrorText(connect_error), true);
            return out;
        }
        ...
        {
            out.error =
                MakeError(httpx::ErrorKind::Network, "connect wait failed: " + SocketErrorText(wait_error), true);
            return out;
        }
        ...
        {
            out.error =
                MakeError(httpx::ErrorKind::Network, "connect failed after wait: " + SocketErrorText(so_error), true);
            return out;
        }
    }
    ...
    {
        out.error = MakeError(httpx::ErrorKind::Network,
                              "restore blocking mode failed: " + SocketErrorText(LastSocketError()), true);
        return out;
    }
...
httpx::Result<SocketHandle> ConnectWithResolvedEndpoints(const ParsedUrl& route, const httpx::ClientOptions& options,
...
{
    const bool default_port = (url.scheme == "http" && url.port == 80) || (url.scheme == "https" && url.port == 443);
    if (default_port)
    {
        return url.host;
    }
...
httpx::Result<std::size_t> RecvSome(SocketHandle socket, char* buffer, std::size_t buffer_size,
                                    const httpx::ClientOptions& options,
                                    std::chrono::steady_clock::time_point deadline);

httpx::Status SendRawBytes(SocketHandle socket, std::string_view payload, const httpx::ClientOptions& options,
                           std::chrono::steady_clock::time_point deadline)
{
    httpx::Status st;
    ...
    while (sent < payload.size())
    {
        ...
        {
            st.ok = false;
            st.error = MakeError(httpx::ErrorKind::Network, "write wait failed: " + SocketErrorText(wait_error), true);
            return st;
        }

#if defined(_WIN32)
        const int n = ::send(socket, payload.data() + sent, static_cast<int>(payload.size() - sent), 0);
#else
        const int n = static_cast<int>(::send(socket, payload.data() + sent, payload.size() - sent, 0));
#endif
        if (n <= 0)
        {
            st.ok = false;
            st.error = MakeError(httpx::ErrorKind::Network, "send failed: " + SocketErrorText(LastSocketError()), true);
            return st;
        }
...
httpx::Result<std::string> RecvExactBytes(SocketHandle socket, std::size_t bytes, const httpx::ClientOptions& options,
                                          std::chrono::steady_clock::time_point deadline)
{
    ...
    while (out.value.size() < bytes)
    {
        ...
        if (received.value == 0)
        {
            out.error = MakeError(httpx::ErrorKind::Network, "connection closed during proxy handshake", true);
            return out;
        }
...
httpx::Status EstablishSocks5NoAuthTunnel(SocketHandle socket, const ParsedUrl& destination,
                                          const httpx::ClientOptions& options,
                                          std::chrono::steady_clock::time_point deadline)
{
...
std::string Base64Encode(std::string_view plain)
{
    ...
    {
        const std::uint32_t block = (static_cast<std::uint32_t>(static_cast<unsigned char>(plain[i])) << 16u) |
...
void MaybeInjectProxyAuthorization(httpx::Request* request, const httpx::ProxyOptions& proxy)
{
    ...
    if ((proxy.username.empty() && proxy.password.empty()) || ContainsHeader(request->headers, "proxy-authorization"))
    {
        return;
    }
...
httpx::Result<std::string> BuildMultipartBody(const httpx::Request& request, std::string* boundary_out)
{
    ...
    {
        out.error =
            MakeError(httpx::ErrorKind::InvalidArgument, "request.body and request.multipart cannot be used together");
        return out;
    }
    ...
    for (const auto& part : request.multipart)
    {
        if (part.name.empty())
        {
            out.error = MakeError(httpx::ErrorKind::InvalidArgument, "multipart part name cannot be empty");
            return out;
        }
...
httpx::Result<std::string> BuildRequestPayload(const httpx::Request& request, const ParsedUrl& url,
                                               const httpx::ClientOptions& options, bool use_proxy_absolute_form)
{
    httpx::Result<std::string> out;
    ...
    const std::string request_target =
        use_proxy_absolute_form ? BuildAbsoluteUriForProxyTarget(url) : (url.target.empty() ? "/" : url.target);
...
httpx::Result<std::size_t> RecvSome(SocketHandle socket, char* buffer, std::size_t buffer_size,
                                    const httpx::ClientOptions& options, std::chrono::steady_clock::time_point deadline)
{
    ...
    if (wait_state == WaitState::Error)
    {
        out.error = MakeError(httpx::ErrorKind::Network, "read wait failed: " + SocketErrorText(wait_error), true);
        return out;
    }
    ...
    if (read_size < 0)
    {
        out.error = MakeError(httpx::ErrorKind::Network, "recv failed: " + SocketErrorText(LastSocketError()), true);
        return out;
    }
...
httpx::Status SendAll(SocketHandle socket, std::string_view payload, const httpx::Request& request,
                      const httpx::ClientOptions& options, std::chrono::steady_clock::time_point deadline)
{
    httpx::Status st;
    ...
    while (sent < payload.size())
    {
        ...
        if (request.on_upload_progress)
        {
            request.on_upload_progress(static_cast<std::uint64_t>(sent), static_cast<std::uint64_t>(payload.size()));
        }
    }
...
bool AppendBodyChunk(httpx::Response* response, std::string_view chunk, const httpx::Request& request,
                     const httpx::ClientOptions& options, httpx::Error* error)
{
...
httpx::Result<httpx::Response> ReadHttpResponse(SocketHandle socket, const httpx::Request& request,
                                                const httpx::ClientOptions& options,
                                                std::chrono::steady_clock::time_point deadline)
{
    ...
    if (parsed_headers.value.chunked)
    {
        ...
        auto fill_chunk_wire = [&]() -> bool
        {
            ...
            if (chunk.value == 0)
            {
                out.error = MakeError(httpx::ErrorKind::Protocol, "connection closed during chunked response decode");
                return false;
            }
            ...
            {
                out.error = MakeError(httpx::ErrorKind::Protocol, "chunked response exceeded configured limits");
                return false;
            }
            return true;
        };
        ...
        for (;;)
        {
            ...
            if (!AppendBodyChunk(&out.value, std::string_view(chunk_wire).substr(cursor, chunk_size), request, options,
                                 &append_error))
            {
    ...
    if (parsed_headers.value.has_content_length)
    {
        ...
        httpx::Error append_error;
        if (!AppendBodyChunk(&out.value, std::string_view(remainder).substr(0, take), request, options, &append_error))
        {
            out.error = append_error;
            return out;
        }
        ...
        while (out.value.body.size() < parsed_headers.value.content_length)
        {
            ...
            const auto need = std::min(parsed_headers.value.content_length - out.value.body.size(), chunk.value);
            httpx::Error append_error_loop;
            if (!AppendBodyChunk(&out.value, std::string_view(io_buffer.data(), need), request, options,
                                 &append_error_loop))
            {
    ...
    {
        httpx::Error append_error;
        if (!AppendBodyChunk(&out.value, std::string_view(remainder), request, options, &append_error))
        {
            out.error = append_error;
            return out;
        }
    ...
    for (;;)
    {
        ...
        httpx::Error append_error_loop;
        if (!AppendBodyChunk(&out.value, std::string_view(io_buffer.data(), chunk.value), request, options,
                             &append_error_loop))
        {
            out.error = append_error_loop;
            return out;
...
httpx::Result<httpx::Response> ReadHttpResponseWithCustomRecv(const httpx::Request& request,
                                                              const httpx::ClientOptions& options, RecvFn recv_fn)
{
    httpx::Result<httpx::Response> out;
...
httpx::Status SendAllWithCustomWriter(std::string_view payload, const httpx::Request& request, WriteFn write_fn)
{
    httpx::Status st;
...
httpx::Error RewriteProxyError(httpx::Error error, bool using_proxy)
{
    ...
    if (error.kind == httpx::ErrorKind::Network || error.kind == httpx::ErrorKind::Timeout ||
...
httpx::Result<std::size_t> OpenSslReadSome(SSL* ssl, SocketHandle socket, char* buffer, std::size_t buffer_size,
                                           const httpx::ClientOptions& options,
                                           std::chrono::steady_clock::time_point deadline)
{
    httpx::Result<std::size_t> out;
    ...
    for (;;)
    {
        ...
        if (err == SSL_ERROR_WANT_READ || err == SSL_ERROR_WANT_WRITE)
        {
            ...
            {
                out.error =
                    MakeError(httpx::ErrorKind::Network, "tls read wait failed: " + SocketErrorText(wait_error), true);
                return out;
            }
        ...
        out.error = MakeError(httpx::ErrorKind::Tls, "tls read failed: " + OpenSslLastErrorText(), true);
        return out;
    }
}

httpx::Result<std::size_t> OpenSslWriteSome(SSL* ssl, SocketHandle socket, const char* buffer, std::size_t buffer_size,
                                            const httpx::ClientOptions& options,
                                            std::chrono::steady_clock::time_point deadline)
{
    httpx::Result<std::size_t> out;
    ...
    for (;;)
    {
        ...
        if (err == SSL_ERROR_WANT_READ || err == SSL_ERROR_WANT_WRITE)
        {
            ...
            {
                out.error =
                    MakeError(httpx::ErrorKind::Network, "tls write wait failed: " + SocketErrorText(wait_error), true);
                return out;
            }
        ...
        out.error = MakeError(httpx::ErrorKind::Tls, "tls write failed: " + OpenSslLastErrorText(), true);
        return out;
    }
}

httpx::Result<httpx::Response> SendByOpenSslTransport(const httpx::Request& request, const ParsedUrl& parsed_url,
                                                      const httpx::ClientOptions& options,
                                                      std::chrono::steady_clock::time_point deadline)
{
    httpx::Result<httpx::Response> out;
    ...
    if (options.tls.verify_peer)
    {
        if (options.tls.ca_file.empty())
        {
            ...
            {
                out.error =
                    MakeError(httpx::ErrorKind::Tls, "load default trust store failed: " + OpenSslLastErrorText());
                return out;
            }
        }
        else if (SSL_CTX_load_verify_locations(session.ctx, options.tls.ca_file.c_str(), nullptr) != 1)
        {
            out.error = MakeError(httpx::ErrorKind::Tls, "load tls.ca_file failed: " + OpenSslLastErrorText());
            return out;
        }
    }
    ...
    for (;;)
    {
        ...
        if (err == SSL_ERROR_WANT_READ || err == SSL_ERROR_WANT_WRITE)
        {
            ...
            {
                out.error = MakeError(httpx::ErrorKind::Network,
                                      "tls handshake wait failed: " + SocketErrorText(wait_error), true);
                return out;
            }
        ...
        out.error = MakeError(httpx::ErrorKind::Tls, "tls handshake failed: " + OpenSslLastErrorText(), true);
        return out;
    }
    ...
    if (options.tls.verify_peer)
    {
        ...
        if (vr != X509_V_OK)
        {
            out.error = MakeError(httpx::ErrorKind::Tls, "tls peer verification failed: " + std::to_string(vr));
            return out;
        }
    }
    ...
    const auto sent = SendAllWithCustomWriter(
        payload.value, request, [&](const char* buf, std::size_t len) -> httpx::Result<std::size_t>
        { return OpenSslWriteSome(session.ssl, guard.Get(), buf, len, options, deadline); });
    if (!sent.ok)
    {
    ...
    return ReadHttpResponseWithCustomRecv(
        request, options, [&](char* buf, std::size_t len) -> httpx::Result<std::size_t>
        { return OpenSslReadSome(session.ssl, guard.Get(), buf, len, options, deadline); });
}
#endif
...
httpx::Result<std::size_t> MbedTlsReadSome(mbedtls_ssl_context* ssl, SocketHandle socket, char* buffer,
                                           std::size_t buffer_size, const httpx::ClientOptions& options,
                                           std::chrono::steady_clock::time_point deadline)
{
    httpx::Result<std::size_t> out;
    ...
    for (;;)
    {
        const int rc = mbedtls_ssl_read(ssl, reinterpret_cast<unsigned char*>(buffer), buffer_size);
        if (rc > 0)
        {
            out.ok = true;
            out.value = static_cast<std::size_t>(rc);
        ...
        out.error = MakeError(httpx::ErrorKind::Tls, "tls read failed: " + MbedTlsErrorText(rc), true);
        return out;
    }
}

httpx::Result<std::size_t> MbedTlsWriteSome(mbedtls_ssl_context* ssl, SocketHandle socket, const char* buffer,
                                            std::size_t buffer_size, const httpx::ClientOptions& options,
                                            std::chrono::steady_clock::time_point deadline)
{
    httpx::Result<std::size_t> out;
    ...
    for (;;)
    {
        const int rc = mbedtls_ssl_write(ssl, reinterpret_cast<const unsigned char*>(buffer), buffer_size);
        if (rc > 0)
        {
            out.ok = true;
            out.value = static_cast<std::size_t>(rc);
        ...
        out.error = MakeError(httpx::ErrorKind::Tls, "tls write failed: " + MbedTlsErrorText(rc), true);
        return out;
    }
}

httpx::Result<httpx::Response> SendByMbedTlsTransport(const httpx::Request& request, const ParsedUrl& parsed_url,
                                                      const httpx::ClientOptions& options,
                                                      std::chrono::steady_clock::time_point deadline)
{
    httpx::Result<httpx::Response> out;
    ...
    int rc = mbedtls_ctr_drbg_seed(&session.drbg, mbedtls_entropy_func, &session.entropy,
                                   reinterpret_cast<const unsigned char*>(personal), std::strlen(personal));
    if (rc != 0)
    {
        out.error = MakeError(httpx::ErrorKind::Tls, "mbedtls rng seed failed: " + MbedTlsErrorText(rc));
        return out;
    }
    ...
    if (rc != 0)
    {
        out.error = MakeError(httpx::ErrorKind::Tls, "mbedtls config defaults failed: " + MbedTlsErrorText(rc));
        return out;
    }
    ...
    if (options.tls.verify_peer)
    {
        if (options.tls.ca_file.empty())
        {
            out.error = MakeError(httpx::ErrorKind::Tls, "mbedtls requires tls.ca_file when verify_peer=true");
            return out;
        }
        ...
        if (rc < 0)
        {
            out.error = MakeError(httpx::ErrorKind::Tls, "load tls.ca_file failed: " + MbedTlsErrorText(rc));
            return out;
        }
    ...
    if (rc != 0)
    {
        out.error = MakeError(httpx::ErrorKind::Tls, "mbedtls ssl setup failed: " + MbedTlsErrorText(rc));
        return out;
    }
    ...
    if (options.tls.verify_host)
    {
        ...
        if (rc != 0)
        {
            out.error = MakeError(httpx::ErrorKind::Tls, "mbedtls set hostname failed: " + MbedTlsErrorText(rc));
            return out;
        }
    }

    SocketHandle bio_socket = guard.Get();
    mbedtls_ssl_set_bio(&session.ssl, &bio_socket, MbedTlsBioSend, MbedTlsBioRecv, nullptr);
    ...
    for (;;)
    {
        ...
        out.error = MakeError(httpx::ErrorKind::Tls, "tls handshake failed: " + MbedTlsErrorText(rc), true);
        return out;
    }
    ...
    if (options.tls.verify_peer)
    {
        ...
        if (flags != 0)
        {
            out.error = MakeError(httpx::ErrorKind::Tls, "tls peer verification failed: " + std::to_string(flags));
            return out;
        }
    }
    ...
    const auto sent = SendAllWithCustomWriter(
        payload.value, request, [&](const char* buf, std::size_t len) -> httpx::Result<std::size_t>
        { return MbedTlsWriteSome(&session.ssl, guard.Get(), buf, len, options, deadline); });
    if (!sent.ok)
    {
        out.error = sent.error;
        return out;
    ...
    return ReadHttpResponseWithCustomRecv(
        request, options, [&](char* buf, std::size_t len) -> httpx::Result<std::size_t>
        { return MbedTlsReadSome(&session.ssl, guard.Get(), buf, len, options, deadline); });
}
#endif

httpx::Result<httpx::Response> SendByTlsBackend(const httpx::Request& request, const ParsedUrl& parsed_url,
                                                const httpx::ClientOptions& options,
                                                std::chrono::steady_clock::time_point deadline)
{
#if defined(HTTPX_ENABLE_OPENSSL)
...
httpx::Result<httpx::Response> SendByDefaultTransport(const httpx::Client* owner, const httpx::Request& request,
                                                      const httpx::ClientOptions& options,
                                                      std::chrono::steady_clock::time_point deadline)
{
    httpx::Result<httpx::Response> out;
    ...
    if (options.proxy.enabled)
    {
        ...
        {
            out.error = MakeError(httpx::ErrorKind::Proxy, "proxy is enabled but host/port is not configured");
            return out;
        }
        ...
        {
            out.error = MakeError(httpx::ErrorKind::Proxy, "supported proxy schemes are http and socks5");
            return out;
        }

        if (proxy_scheme == "socks5" && (!options.proxy.username.empty() || !options.proxy.password.empty()))
        {
            out.error = MakeError(httpx::ErrorKind::Proxy, "socks5 no-auth mode does not accept username/password");
            return out;
        }
    ...
    {
        const auto tunnel_status = EstablishSocks5NoAuthTunnel(socket, parsed_url.value, options, deadline);
        if (!tunnel_status.ok)
        {
            out.ok = false;
...
bool ValidateTlsOptionsForHttps(const httpx::ClientOptions& options, httpx::Error* error)
{
    ...
    {
        *error = MakeError(httpx::ErrorKind::InvalidArgument, "tls.verify_host=true requires tls.verify_peer=true");
        return false;
    }
    ...
    if (!options.tls.ca_file.empty())
    {
        ...
        if (!ca_file.good())
        {
            *error = MakeError(httpx::ErrorKind::Tls, "tls.ca_file cannot be opened: " + options.tls.ca_file);
            return false;
        }
    }
...
Result<Response> Client::Send(const Request& request)
{
    ...
    EmitLog(request, last, duration_ms, effective, last.ok ? LogSeverity::Info : LogSeverity::Error);
                                  ...
                                  std::chrono::steady_clock::time_point deadline)
{
    ...
    for (std::size_t redirect_hops = 0;; ++redirect_hops)
    {
        ...
        {
            Result<Response> invalid;
            invalid.error = MakeError(ErrorKind::Redirect, "redirect target is invalid: " + parsed_next.error.message);
            return invalid;
        }
...
void Client::EmitLog(const Request& request, const Result<Response>& result, std::uint64_t duration_ms,
                     const ClientOptions& effective_options, LogSeverity severity) const
{
