...
constexpr ErrorCode kThirdPartyCallFailed =
    LOGSYS_MAKE_ERROR_CODE(ErrorSource::ThirdParty, ModuleId::ThirdPartyAdapter, 1);
...
std::string FormatTime(const std::chrono::system_clock::time_point tp)
{
    ...
    os << std::put_time(&tmv, "%Y-%m-%d %H:%M:%S") << '.' << std::setw(3) << std::setfill('0') << ms;
    return os.str();
}
...
ResolvedProfileV2 ProfileResolverV2::Resolve(const LoggerConfigV2& config, std::string_view file_path, ModuleId module)
{
...
std::optional<LogLevel> ParseLogLevelFromToken(std::string_view token)
{
    ...
    if (std::all_of(cleaned.begin(), cleaned.end(), [](unsigned char c) { return std::isdigit(c) != 0 || c == '-'; }))
    {
...
{
    entries_ = {
        {kSystemIoOpenFailed, ErrorSource::System, ModuleId::Sink, ErrorCategory::Io, "file open failed: %s",
         ActionHint::Fallback, false, 1, "", ""},
...
std::string TextFormatter::Format(const LogEvent& event) const
{
    ...
    std::ostringstream os;
    const auto has = [&](TextField field) { return (event.text_field_mask & static_cast<std::uint32_t>(field)) != 0; };
...
{
    std::ostringstream os;
    os << '{' << "\"time\":\"" << EscapeJson(FormatTime(event.timestamp)) << "\","
       << "\"level\":\"" << ToString(event.level) << "\","
       << "\"code\":" << event.code << ',' << "\"category\":\"" << ToString(event.category) << "\","
       << "\"file\":\"" << EscapeJson(event.file) << "\","
       << "\"line\":" << event.line << ',' << "\"func\":\"" << EscapeJson(event.function) << "\","
    ...
    if (event.ext_count > 0)
    {
        ...
        for (std::size_t i = 0; i < event.ext_count; ++i)
        {
            ...
            os << "\"" << EscapeJson(event.ext_fields[i].key) << "\":\"" << EscapeJson(event.ext_fields[i].value)
               << "\"";
        }
        os << '}';
    }
...
FileSink::FileSink(std::string path, RollingConfigV2 rolling) : path_(std::move(path)), rolling_(rolling)
{
    OpenLocked();
}
...
UdpSyslogSink::UdpSyslogSink(RemoteConfigV2 config)
    : host_(std::move(config.udp_host)), port_(config.udp_port), facility_(config.syslog_facility),
      app_name_(std::move(config.syslog_app_name)), hostname_(std::move(config.syslog_hostname))
{
    if (app_name_.empty())
    {
        app_name_ = "logsys";
...
{
    {
        ...
        formatter_ = options.use_json_formatter
                         ? std::static_pointer_cast<IFormatter>(std::make_shared<JsonFormatter>())
...
void Logger::ConfigureSimpleLogger(LogLevel output_level, bool enable_console, bool enable_file, std::string file_path,
...
void Logger::StartPeriodicFlush(std::chrono::milliseconds interval)
{
    ...
    periodic_flush_thread_ = std::thread(
        [this, effective_interval]()
        {
...
void Logger::StartAsyncWorker()
{
    ...
    async_worker_thread_ = std::thread(
        [this]()
        {
            {
                std::lock_guard<std::mutex> lk(async_mu_);
            ...
            while (true)
            {
                LogEvent event;
                {
                    ...
                    async_cv_.wait(lk, [this] { return async_worker_stop_requested_ || !async_queue_.empty(); });
...
void Logger::FlushGroupedOutputsLocked()
{
    ...
    static constexpr std::array<LogLevel, 7> kOrder = {LogLevel::Trace,   LogLevel::Debug, LogLevel::Info,
    ...
    for (LogLevel ordered_level : kOrder)
    {
        for (const auto& entry : grouped_outputs_)
        {
            ...
            for (const auto& sink : sinks)
            {
                ...
                {
                    sink->Write(
                        ColorizeForConsole(entry.line, entry.level, entry.color_console && entry.light_theme_only));
                }
...
void Logger::Enqueue(LogEvent event)
{
    ...
    if (backpressure.drop_low_level_when_full && pending_now > backpressure.queue_high_watermark &&
...
void Logger::LogEventNow(LogEvent event)
{
    ...
    {
        std::lock_guard<std::mutex> lk(mu_);
        grouped_outputs_.push_back(PendingOutputV2{event.level, line, allow_console, allow_file, allow_debugger,
                                                   enable_color, light_theme_only});
...
{
    {
        ...
        {
            async_drain_cv_.wait(lk, [this] { return async_queue_.empty() && !async_worker_processing_; });
        }
    }
