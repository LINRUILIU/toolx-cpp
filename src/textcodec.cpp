...
DecodeResult<std::string> make_error(DecodeError code, std::string_view key, std::string_view reason)
{
...
std::string base64_encode_impl(std::string_view input, const Base64Options& options)
{
    static constexpr char kTableStd[] = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/";
    static constexpr char kTableUrl[] = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789-_";
...
DecodeResult<std::string> base64_decode_impl(std::string_view input, const Base64Options& options)
{
    ...
    if (!options.padding)
    {
        ...
        if (mod == 1u)
        {
            return make_error(DecodeError::InvalidPadding, "base64_decode",
    ...
    {
        return make_error(DecodeError::InvalidPadding, "base64_decode", "base64 length must be a multiple of 4");
    }

    const auto& table = (options.variant == Base64Variant::UrlSafe) ? base64url_decode_table() : base64_decode_table();
    ...
    for (std::size_t i = 0; i < normalized.size(); i += 4u)
    {
        ...
        if (c0 == '=' || c1 == '=')
        {
            return make_error(DecodeError::InvalidPadding, "base64_decode", "invalid padding position");
        }
        ...
        if (v0 < 0 || v1 < 0)
        {
            return make_error(DecodeError::InvalidCharacter, "base64_decode", "invalid base64 character");
        }
        ...
        if (pad2)
        {
            ...
            {
                return make_error(DecodeError::InvalidPadding, "base64_decode", "invalid final block padding");
            }

            const std::uint32_t x = (static_cast<std::uint32_t>(v0) << 18u) | (static_cast<std::uint32_t>(v1) << 12u);
        ...
        if (pad3)
        {
            ...
            {
                return make_error(DecodeError::InvalidPadding, "base64_decode", "padding only allowed in final block");
            }
...
DecodeResult<std::string> hex_decode(std::string_view input)
{
    if ((input.size() % 2u) != 0u)
    {
        return make_error(DecodeError::TruncatedInput, "hex_decode", "hex input length must be even");
    }
    ...
    for (std::size_t i = 0; i < input.size(); i += 2u)
    {
        ...
        if (hi < 0 || lo < 0)
        {
            return make_error(DecodeError::InvalidCharacter, "hex_decode", "invalid hex character");
        }
...
DecodeResult<std::size_t> hex_decode_to_buffer(std::string_view input, void* out_buffer, std::size_t out_capacity)
{
...
DecodeResult<std::string> url_decode(std::string_view input, const UrlDecodeOptions& options)
{
    ...
    for (std::size_t i = 0; i < input.size(); ++i)
    {
        ...
        if (i + 2u >= input.size())
        {
            return make_error(DecodeError::TruncatedInput, "url_decode", "truncated percent-encoding sequence");
        }
        ...
        if (hi < 0 || lo < 0)
        {
            return make_error(DecodeError::InvalidPercentEncoding, "url_decode", "invalid percent-encoding sequence");
        }
