...
{
    return (cp >= 0x1100 && cp <= 0x115F) || (cp >= 0x2329 && cp <= 0x232A) || (cp >= 0x2E80 && cp <= 0xA4CF) ||
...
std::string join_context(std::string_view scope, std::string_view key, std::string_view reason)
{
    std::string out;
...
std::string format_error(std::string_view scope, std::string_view key, std::string_view reason)
{
...
{
    return text.size() >= suffix.size() && text.substr(text.size() - suffix.size(), suffix.size()) == suffix;
}
